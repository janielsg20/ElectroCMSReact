  ...
  if (type === 'core/map') {
    ...
    return (
      <div className="record-map-control">
        <input aria-label={`${field.label} latitude`} type="number" min={-90} max={90} step="any" placeholder="Latitude" value={lat} onChange={(event) => updatePoint('lat', event.target.value)} />
        <input aria-label={`${field.label} longitude`} type="number" min={-180} max={180} step="any" placeholder="Longitude" value={lng} onChange={(event) => updatePoint('lng', event.target.value)} />
      </div>
    );
  }
...
export function RecordsEditor() {
    ...
    setDraft((current) => {
      ...
      if (checked && !current.fieldGroupIds.includes(group.id)) {
        return {
          ...
          fieldValues: { ...current.fieldValues, [group.id]: defaultGroupValues(group, fieldGroups) },
        };
      }
      if (!checked) {
  ...
  const saveDraft = () => {
    ...
    const candidate = mode === 'create' ? draft : { ...draft, updatedAt: new Date().toISOString() };
  ...
  return (
    <section className="records-editor" aria-label="Records">
      ...
      <div className="records-filters" aria-label="Record filters">
        <label><span>Search</span><input aria-label="Search records" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Title or slug" /></label>
        <label><span>Content Type</span><select aria-label="Filter records by content type" value={contentTypeFilter} onChange={(event) => setContentTypeFilter(event.target.value)}><option value="all">All content types</option>{contentTypes.map((contentType) => <option key={contentType.id} value={contentType.id}>{contentType.label}</option>)}</select></label>
        <label><span>Status</span><select aria-label="Filter records by status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'all' | ContentRecordStatus)}><option value="all">All statuses</option>{CONTENT_RECORD_STATUSES.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
      </div>
      ...
      <div className="records-editor-grid">
        <aside className="records-list" aria-label="Record list">
          ...
          {records.length === 0 ? <div className="records-empty-list"><strong>No records match</strong><span>Create content or adjust the current filters.</span></div> : (
            <div className="records-list-items">
              {records.map((record) => {
                ...
                return <button key={record.id} type="button" className="record-list-item" data-selected={selected ? 'true' : 'false'} aria-pressed={selected} onClick={() => selectRecord(record)}><span className="record-list-main"><strong>{record.title || record.id}</strong><code>/{record.slug}</code></span><span className="record-list-meta"><span>{contentType?.singularLabel ?? record.contentTypeId}</span><span data-status={record.status}>{record.status}</span></span></button>;
              })}
            </div>
          )}
        </aside>
        ...
        <div className="record-detail">
          {!draft ? (
            <div className="record-detail-empty"><span aria-hidden="true">R</span><strong>Select a record or create a new one</strong><p>The editor resolves its schema from the selected CPT and field groups.</p><button type="button" onClick={beginCreate} disabled={contentTypes.length === 0}>Create record</button></div>
          ) : (
            <form className="record-form" onSubmit={(event) => { event.preventDefault(); saveDraft(); }}>
              <div className="record-form-toolbar">
                ...
                <div className="record-form-actions">{mode === 'edit' ? <button type="button" className="record-delete" data-armed={deleteArmed ? 'true' : 'false'} onClick={deleteDraft}>{deleteArmed ? 'Confirm delete' : 'Delete'}</button> : null}<button type="submit" className="record-save" disabled={!validation?.ok}>{mode === 'create' ? 'Create record' : 'Save changes'}</button></div>
              </div>
              ...
              <section className="record-form-section">
                ...
                {fieldGroups.length === 0 ? <div className="record-scope-note">No Custom Field Groups exist yet. Records can still use core content fields.</div> : <div className="record-group-selector" aria-label="Record field groups">{fieldGroups.map((group) => <label key={group.id}><input type="checkbox" checked={draft.fieldGroupIds.includes(group.id)} onChange={(event) => toggleFieldGroup(group, event.target.checked)} /><span><strong>{group.label}</strong><small>{group.fields.length} {group.fields.length === 1 ? 'field' : 'fields'} · {group.presentation}</small></span></label>)}</div>}
              </section>
                        ...
                        return (
                          <div className={`record-field ${wide ? 'record-field--wide' : ''}`} key={field.id}>
                            ...
                            {advanced ? (
                              ...
                              <FieldValueControl field={field} value={value} onChange={(next) => patchFieldValue(group.id, field.name, next)} />
                            )}
                            <small data-error={issue ? 'true' : 'false'}>{issue ?? field.description ?? `${field.type}@${field.typeVersion}`}</small>
                          </div>
                        );
                      })}
                    </div>

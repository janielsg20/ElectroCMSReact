...
export function ReferenceRecordFieldControl({ project, ownerContentTypeId, field, value, onChange }: ReferenceRecordFieldControlProps) {
  if (field.type === 'core/user') {
    ...
    return <select className="ec-control h-8 w-full px-2 text-[10px]" aria-label={field.label} value={selected} onChange={(event) => onChange(event.target.value || null)}>
      <option value="">No user</option>{users.map(([id, raw]) => <option key={id} value={id}>{userLabel(id, raw)} · {id}</option>)}
    </select>;
  }
  ...
  if (field.type === 'core/taxonomy') {
    ...
    return <div className="space-y-1">
      <input className="ec-control h-8 w-full px-2 text-[10px]" aria-label={field.label} value={ids.join(', ')} placeholder="term-id, second-term-id" onChange={(event) => onChange([...new Set(event.target.value.split(',').map((item) => item.trim()).filter(Boolean))])} />
      <small className="block text-[8px] leading-4 text-[var(--color-ec-text-muted)]">{taxonomy ? `${taxonomy.label} · scoped term IDs. Canonical term catalog is not modeled yet.` : `Configured taxonomy ${taxonomyId || '(empty)'} is unavailable.`}</small>
    </div>;
  }
  ...
  if (cardinality === 'one') return <div className="space-y-1">
    <select className="ec-control h-8 w-full px-2 text-[10px]" aria-label={field.label} value={selected[0] ?? ''} onChange={(event) => onChange(event.target.value ? [event.target.value] : [])}>
      <option value="">No related record</option>{records.map((record) => <option key={record.id} value={record.id}>{record.title || record.id} · /{record.slug}</option>)}
    </select><small className="block text-[8px] text-[var(--color-ec-text-muted)]">{relation.label} · one {contentType?.singularLabel ?? referencedContentTypeId}</small>
  </div>;

  return <fieldset className="space-y-1 rounded-[var(--ec-radius-sm)] border border-[var(--color-ec-border)] p-2" aria-label={field.label}>
    ...
    {records.length === 0 ? <small className="text-[8px] text-[var(--color-ec-text-muted)]">No compatible records exist yet.</small> : records.map((record) => {
      const checked = selected.includes(record.id);
      return <label key={record.id} className="flex items-center gap-2 text-[9px] text-[var(--color-ec-text)]"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked ? [...selected, record.id] : selected.filter((id) => id !== record.id))} /><span><strong>{record.title || record.id}</strong><small className="ml-1 text-[var(--color-ec-text-muted)]">/{record.slug}</small></span></label>;
    })}
  </fieldset>;
}

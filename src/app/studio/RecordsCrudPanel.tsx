...
function resolvedGroupValues(record: ContentRecordDefinition, group: FieldGroupDefinition): JsonObject {
  const stored = groupValues(record, group.id);
  const resolved: JsonObject = { ...stored };
  for (const field of group.fields) {
    if (resolved[field.name] === undefined) resolved[field.name] = structuredClone(field.defaultValue);
  }
  return resolved;
}

function updateGroupValue(record: ContentRecordDefinition, groupId: string, fieldName: string, value: JsonValue): ContentRecordDefinition {
  return {
    ...record,
    fieldValues: {
      ...record.fieldValues,
...
function RecordFieldEditor({
    ...
    return (
      <div className="sm:col-span-2">
        ...
        <AdvancedRecordFieldControl
          ...
          onChange={(next) => onChange(updateGroupValue(draft, group.id, field.name, next))}
        />
        {field.description ? <small className="mt-1 block text-[8px] leading-4 text-[var(--color-ec-text-muted)]">{field.description}</small> : null}
      </div>
    );
  }
  ...
  if (definition.valueShape === 'string') {
    ...
    return (
      <label className="block text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
        ...
        {multiline ? (
          <textarea className="ec-control min-h-20 w-full resize-y px-2.5 py-2 text-[10px]" aria-label={field.label} placeholder={field.placeholder ?? undefined} value={typeof value === 'string' ? value : ''} onChange={(event) => onChange(updateGroupValue(draft, group.id, field.name, event.target.value))} />
        ) : (
          <input className="ec-control h-8 w-full px-2 text-[10px]" aria-label={field.label} placeholder={field.placeholder ?? undefined} value={typeof value === 'string' ? value : ''} onChange={(event) => onChange(updateGroupValue(draft, group.id, field.name, event.target.value))} />
        )}
  ...
  if (definition.valueShape === 'number') {
    return (
      <label className="block text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
        <span className="mb-1 flex items-center justify-between gap-2"><span>{label}</span><small className="font-mono font-normal opacity-70">{field.name}</small></span>
        <input className="ec-control h-8 w-full px-2 text-[10px]" aria-label={field.label} type="number" step="any" value={typeof value === 'number' ? String(value) : ''} onChange={(event) => onChange(updateGroupValue(draft, group.id, field.name, event.target.value === '' ? null : Number(event.target.value)))} />
      </label>
    );
  }
  ...
  if (definition.valueShape === 'boolean') {
    return (
      <label className="flex min-h-9 items-center gap-2 rounded-[var(--ec-radius-sm)] border border-[var(--color-ec-border)] px-2.5 text-[10px] font-semibold text-[var(--color-ec-text)]">
        <input type="checkbox" aria-label={field.label} checked={value === true} onChange={(event) => onChange(updateGroupValue(draft, group.id, field.name, event.target.checked))} />
        <span>{label}</span><small className="ml-auto font-mono font-normal text-[var(--color-ec-text-muted)]">{field.name}</small>
      </label>
    );
  }
  ...
  return (
    <label className="block text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
      ...
      <textarea key={JSON.stringify(value)} className="ec-control min-h-20 w-full resize-y px-2.5 py-2 font-mono text-[9px]" aria-label={field.label} defaultValue={JSON.stringify(value, null, 2)} onBlur={(event) => { try { onChange(updateGroupValue(draft, group.id, field.name, JSON.parse(event.target.value) as JsonValue)); } catch { /* keep last valid value */ } }} />
    </label>
  );
}
...
export function RecordsCrudPanel({ query }: RecordsCrudPanelProps) {
  ...
  const beginCreate = () => {
    ...
    if (!first) return;
    setCreating(true); setSelectedId(null); setDraft(createDraft(session.project, first.id)); setMessage(null); setDeleteArmed(false);
  };
  const selectRecord = (record: ContentRecordDefinition) => { setCreating(false); setSelectedId(record.id); setDraft(structuredClone(record)); setMessage(null); setDeleteArmed(false); };
  const updateDraft = <K extends keyof ContentRecordDefinition>(key: K, value: ContentRecordDefinition[K]) => { setDraft((current) => current ? { ...current, [key]: value } : current); setMessage(null); };
  const changeContentType = (contentTypeId: string) => { if (!creating) return; setDraft(createDraft(session.project, contentTypeId)); setMessage(null); };
  ...
  const toggleFieldGroup = (group: FieldGroupDefinition, checked: boolean) => {
    setDraft((current) => {
      ...
      if (checked) {
        ...
        return { ...current, fieldGroupIds: [...current.fieldGroupIds, group.id], fieldValues: { ...current.fieldValues, [group.id]: current.fieldValues[group.id] ?? {} } };
      }
      const fieldValues = { ...current.fieldValues }; delete fieldValues[group.id];
  ...
  const save = () => {
    if (!draft) return;
    const result = creating ? session.createContentRecord(draft) : selected ? session.updateContentRecord(selected.id, draft) : null;
    if (!result) return;
    if (!result.ok) { setMessage({ tone: 'error', text: result.message }); return; }
    setCreating(false); setSelectedId(result.value.id); setDraft(structuredClone(result.value)); setMessage({ tone: 'success', text: result.changed ? 'Record saved.' : 'No changes to save.' });
  };
  const remove = () => {
    if (!selected) return;
    ...
    setDeleteArmed(false); setSelectedId(null); setDraft(null); setMessage({ tone: 'success', text: `Deleted ${selected.title || selected.slug}.` });
  };
    ...
    <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1fr)_560px]">
      <div className="min-h-0 overflow-y-auto">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          <select className="ec-control h-8 min-w-36 px-2 text-[10px]" aria-label="Filter records by content type" value={contentTypeFilter} onChange={(event) => setContentTypeFilter(event.target.value)}><option value="">All content types</option>{contentTypes.map((definition) => <option key={definition.id} value={definition.id}>{definition.label}</option>)}</select>
          <select className="ec-control h-8 min-w-32 px-2 text-[10px]" aria-label="Filter records by status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as ContentRecordStatus | '')}><option value="">All statuses</option>{CONTENT_RECORD_STATUSES.map((status) => <option key={status} value={status}>{status}</option>)}</select>
        ...
        {records.length ? (
          <div className="overflow-hidden rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] shadow-[var(--ec-shadow-panel)]">
            ...
            {records.map((record) => {
              ...
              return <button key={record.id} type="button" className="grid min-h-12 w-full grid-cols-[minmax(160px,1.2fr)_minmax(110px,.8fr)_90px_110px] items-center gap-3 border-b border-[var(--color-ec-border)] px-3 text-left last:border-0 hover:bg-[var(--color-ec-surface-subtle)] data-[active=true]:bg-[var(--color-ec-accent-soft)]" data-active={!creating && selected?.id === record.id ? 'true' : 'false'} aria-label={`${record.title || record.slug} ${record.slug} ${record.status}`} onClick={() => selectRecord(record)}><span className="min-w-0"><strong className="block truncate text-[10px] text-[var(--color-ec-text)]">{record.title || record.slug}</strong><small className="font-mono text-[8px] text-[var(--color-ec-text-muted)]">{record.id}</small></span><span className="truncate text-[9px] text-[var(--color-ec-text-muted)]">{typeLabel}</span><span className="text-[9px] font-semibold capitalize text-[var(--color-ec-text-muted)]">{record.status}</span><time className="text-[8px] text-[var(--color-ec-text-muted)]">{new Date(record.updatedAt).toLocaleDateString()}</time></button>;
            })}
          </div>
        ) : <div className="grid min-h-64 place-items-center rounded-[var(--ec-radius-lg)] border border-dashed border-[var(--color-ec-border)] bg-[var(--color-ec-surface-subtle)] text-center"><div><Icon name="list" size={22} /><strong className="mt-3 block text-[12px] text-[var(--color-ec-text)]">{Object.keys(session.project.records).length ? 'No records match' : 'No records yet'}</strong><p className="mt-1 text-[10px] text-[var(--color-ec-text-muted)]">{contentTypes.length ? 'Create a canonical record or change the active filters.' : 'Create a Content Type before adding records.'}</p></div></div>}
      </div>
      ...
      <aside className="min-h-0 overflow-y-auto rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] shadow-[var(--ec-shadow-panel)]" aria-label="Record editor">
        ...
        {draft ? (
          <div className="space-y-4 p-3">
            ...
            <fieldset className="rounded-[var(--ec-radius-md)] border border-[var(--color-ec-border)] p-3"><legend className="px-1 text-[9px] font-bold uppercase tracking-[.1em] text-[var(--color-ec-text-muted)]">Field Groups</legend>{fieldGroups.length ? <div className="grid gap-2 sm:grid-cols-2">{fieldGroups.map((group) => <label key={group.id} className="flex min-h-9 items-center gap-2 rounded-[var(--ec-radius-sm)] border border-[var(--color-ec-border)] px-2.5 text-[10px] text-[var(--color-ec-text)]"><input type="checkbox" aria-label={`Record field group ${group.label}`} checked={draft.fieldGroupIds.includes(group.id)} onChange={(event) => toggleFieldGroup(group, event.target.checked)} /><span className="min-w-0"><strong className="block truncate">{group.label}</strong><small className="font-mono text-[8px] text-[var(--color-ec-text-muted)]">{group.fields.length} fields</small></span></label>)}</div> : <p className="text-[9px] text-[var(--color-ec-text-muted)]">No reusable Field Groups are defined yet.</p>}</fieldset>

...
export function StudioCommandPalette({ open, onOpenChange, onNavigate, onSelectModule }: StudioCommandPaletteProps) {
  ...
  const commands = useMemo<CommandItem[]>(() => {
    ...
    return [
      ...
      { id: 'module-themes', label: 'Site Design', detail: 'Frontend and backend project design packages', icon: 'theme', run: close(() => onSelectModule('themes')) },
      { id: 'module-settings', label: 'Settings', detail: 'Project, storage and fixed interface information', icon: 'settings', run: close(() => onSelectModule('settings')) },
    ];
  ...
  return (
    <div className="fixed inset-0 z-[90] grid place-items-start bg-black/55 px-3 pt-[10vh] backdrop-blur-[3px]" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) closePalette(); }}>
      <section className="mx-auto w-full max-w-[620px] overflow-hidden rounded-[var(--ec-radius-xl)] border border-[var(--color-ec-border-strong)] bg-[var(--color-ec-surface)] shadow-[var(--ec-shadow-float)] [animation:ec-pop-enter_var(--ec-motion-normal)_var(--ec-ease-spring)]" role="dialog" aria-modal="true" aria-label="Command palette">
        <label className="flex h-12 items-center gap-2 border-b border-[var(--color-ec-border)] px-3 text-[var(--color-ec-text-muted)]"><Icon name="search" size={15} /><span className="sr-only">Search commands</span><input autoFocus className="min-w-0 flex-1 bg-transparent text-[11px] text-[var(--color-ec-text)] outline-none placeholder:text-[var(--color-ec-text-faint)]" aria-label="Search commands" placeholder="Search workspaces and tools…" value={query} onChange={(event) => setQuery(event.target.value)} /><kbd className="rounded border border-[var(--color-ec-border)] bg-[var(--color-ec-surface-subtle)] px-1.5 py-0.5 text-[8px]">Esc</kbd></label>
        <nav className="max-h-[min(62vh,520px)] overflow-y-auto p-2" aria-label="Available commands">
          {visible.map((command) => <button key={command.id} type="button" className="ec-focus-ring ec-action flex min-h-11 w-full items-center gap-3 rounded-[var(--ec-radius-md)] px-2.5 text-left hover:bg-[var(--color-ec-surface-muted)]" onClick={command.run}><span className="grid size-7 shrink-0 place-items-center rounded-[var(--ec-radius-sm)] bg-[var(--color-ec-surface-subtle)] text-[var(--color-ec-text-muted)]"><Icon name={command.icon} size={13} /></span><span className="min-w-0 flex-1"><strong className="block truncate text-[9px] font-semibold text-[var(--color-ec-text)]">{command.label}</strong><small className="block truncate text-[8px] text-[var(--color-ec-text-muted)]">{command.detail}</small></span>{command.shortcut ? <kbd className="rounded border border-[var(--color-ec-border)] px-1.5 py-0.5 text-[8px] text-[var(--color-ec-text-muted)]">{command.shortcut}</kbd> : null}</button>)}

...
const moduleFeatures: Record<Exclude<StudioModuleId, 'builder' | 'themes' | 'pages' | 'media'>, readonly FeatureGroup[]> = {
  content: [
  ...
  return (
    <aside className={`workspace-navigation studio-rail flex h-full shrink-0 flex-col border-white/[.06] bg-[var(--color-ec-chrome)] p-2 text-white ${collapsed ? 'w-[58px]' : 'w-[216px]'} ${preferences.navigationPosition === 'right' ? 'border-l' : 'border-r'}`} aria-label="Workspace navigation" data-position={preferences.navigationPosition} data-collapsed={collapsed ? 'true' : 'false'} data-display-mode={displayMode}>
      <div className="mb-2 flex h-10 items-center justify-between gap-2 px-1">
        <div className="flex min-w-0 items-center gap-2"><div className="grid size-8 shrink-0 place-items-center rounded-[var(--ec-radius-md)] bg-[var(--color-ec-chrome-raised)] text-[var(--color-ec-accent)] ring-1 ring-white/[.07]"><Icon name="bolt" size={16} /></div>{showLabels ? <div className="min-w-0"><strong className="block truncate text-[12px] font-semibold tracking-[-.01em] text-white">ElectroCMS</strong><span className="block text-[8px] font-semibold uppercase tracking-[.18em] text-white/35">Workspace</span></div> : null}</div>
      ...
      <nav className="space-y-1" aria-label="Primary workspaces">{preferences.workspaceOrder.map((workspaceIdFromOrder) => { const workspace = primaryWorkspaces.find((item) => item.id === workspaceIdFromOrder); if (!workspace) return null; return <button key={workspace.id} type="button" className={railButtonClass} data-active={workspaceId === workspace.id ? 'true' : 'false'} aria-label={workspace.label} title={workspace.label} onClick={() => { onNavigate(workspace.id); if (compactLayout) onClose(); }}>{workspaceId === workspace.id ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-[var(--color-ec-accent)]" aria-hidden="true" /> : null}{showIcons ? <span className="grid size-6 shrink-0 place-items-center"><Icon name={workspace.icon} size={15} /></span> : null}{showLabels ? <span className="min-w-0 flex-1 truncate">{workspace.label}</span> : null}</button>; })}</nav>
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5 [scrollbar-width:thin] [scrollbar-color:rgba(255,255,255,.12)_transparent]" aria-label="Studio modules">{modules.map((module) => <button key={module.id} type="button" className={railButtonClass} data-active={workspaceId === 'editor' && activeModule === module.id ? 'true' : 'false'} aria-label={module.label} title={module.label} onClick={() => { onSelectModule(module.id); if (compactLayout) onClose(); }}>{workspaceId === 'editor' && activeModule === module.id ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-[var(--color-ec-accent)]" aria-hidden="true" /> : null}{showIcons ? <span className="grid size-6 shrink-0 place-items-center"><Icon name={module.icon} size={15} /></span> : null}{showLabels ? <span className="min-w-0 flex-1 truncate">{module.label}</span> : null}</button>)}</nav>
      <details className="group/settings relative mt-2 border-t border-white/[.06] pt-2" open={settingsOpen} onToggle={(event) => onSettingsOpenChange(event.currentTarget.open)}>
        ...
        <div className={`absolute bottom-0 z-50 w-[264px] rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-3 text-[var(--color-ec-text)] shadow-[var(--ec-shadow-float)] ${preferences.navigationPosition === 'right' ? 'right-full mr-2' : 'left-full ml-2'}`}>
          ...
          <div className="grid grid-cols-2 gap-2"><label className="text-[9px] font-medium text-[var(--color-ec-text-muted)]"><span className="mb-1 block">Position</span><select className="ec-control h-8 w-full px-2 text-[10px]" aria-label="Navigation position" value={preferences.navigationPosition} onChange={(event) => setNavigationPosition(event.target.value as 'left' | 'right')}><option value="left">Left</option><option value="right">Right</option></select></label><label className="text-[9px] font-medium text-[var(--color-ec-text-muted)]"><span className="mb-1 block">Display</span><select className="ec-control h-8 w-full px-2 text-[10px]" aria-label="Navigation display mode" value={preferences.navigationDisplayMode} onChange={(event) => setNavigationDisplayMode(event.target.value as 'icons' | 'labels' | 'both')}><option value="both">Icons + labels</option><option value="icons">Icons</option><option value="labels">Labels</option></select></label><label className="col-span-2 text-[9px] font-medium text-[var(--color-ec-text-muted)]"><span className="mb-1 block">Density</span><select className="ec-control h-8 w-full px-2 text-[10px]" aria-label="Workspace density" value={preferences.density} onChange={(event) => setDensity(event.target.value as 'compact' | 'comfortable')}><option value="compact">Compact</option><option value="comfortable">Comfortable</option></select></label></div>
...
function ModuleWorkspace({ module }: { module: Exclude<StudioModuleId, 'builder' | 'themes' | 'pages' | 'media'> }) { const definition = modules.find((item) => item.id === module); return <section className="min-h-0 flex-1 overflow-y-auto bg-[var(--color-ec-app)] p-4 md:p-5" aria-label={`${definition?.label ?? module} workspace`}><header className="mx-auto flex max-w-[1280px] flex-col gap-3 border-b border-[var(--color-ec-border)] pb-4 sm:flex-row sm:items-end sm:justify-between"><div><span className="text-[8px] font-semibold uppercase tracking-[.18em] text-[var(--color-ec-accent)]">ElectroCMS</span><h2 className="mt-1 text-xl font-semibold tracking-[-.03em] text-[var(--color-ec-text)]">{definition?.label ?? module}</h2><p className="mt-1 max-w-2xl text-[10px] leading-5 text-[var(--color-ec-text-muted)]">{definition?.description}</p></div><button type="button" className={quietButton}><Icon name="more" size={13} />Options</button></header><div className="mx-auto mt-5 grid max-w-[1280px] gap-3 lg:grid-cols-3">{moduleFeatures[module].map((group) => <FeatureCard key={group.title} {...group} />)}</div></section>; }
function ThemesWorkspace() { return <section className="min-h-0 flex-1 overflow-y-auto bg-[var(--color-ec-app)] p-4 md:p-5" aria-label="Themes workspace"><header className="mx-auto max-w-[1280px] border-b border-[var(--color-ec-border)] pb-4"><span className="text-[8px] font-semibold uppercase tracking-[.18em] text-[var(--color-ec-accent)]">Design systems</span><h2 className="mt-1 text-xl font-semibold tracking-[-.03em] text-[var(--color-ec-text)]">Theme Studio</h2><p className="mt-1 text-[10px] leading-5 text-[var(--color-ec-text-muted)]">Manage the frontend and backend theme packages connected to canonical project state.</p></header><div className="mx-auto mt-5 grid max-w-[1280px] gap-4 xl:grid-cols-2"><ProjectThemeControls scope="frontend" /><ProjectThemeControls scope="backend" /></div></section>; }
function PreviewWorkspace() { return <section className="min-h-0 flex-1 overflow-y-auto bg-[var(--color-ec-app)] p-3 md:p-5" aria-label="Preview workspace"><div className="mx-auto flex h-full max-w-[1440px] flex-col gap-3"><div className="flex items-center justify-between"><div><span className="text-[8px] font-semibold uppercase tracking-[.18em] text-[var(--color-ec-accent)]">Frontend preview</span><h2 className="mt-0.5 text-lg font-semibold tracking-[-.02em] text-[var(--color-ec-text)]">Preview workspace</h2></div><div className="flex rounded-[var(--ec-radius-md)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-0.5"><button className="h-7 rounded-[var(--ec-radius-sm)] bg-[var(--color-ec-chrome)] px-2.5 text-[9px] font-semibold text-white" type="button">Desktop</button><button className="h-7 rounded-[var(--ec-radius-sm)] px-2.5 text-[9px] font-semibold text-[var(--color-ec-text-muted)]" type="button">Tablet</button><button className="h-7 rounded-[var(--ec-radius-sm)] px-2.5 text-[9px] font-semibold text-[var(--color-ec-text-muted)]" type="button">Mobile</button></div></div><div className="grid min-h-[620px] flex-1 gap-3 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="overflow-hidden rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] shadow-[var(--ec-shadow-panel)]"><div className="flex h-9 items-center gap-1.5 border-b border-[var(--color-ec-border)] bg-[var(--color-ec-surface-subtle)] px-3"><span className="size-2 rounded-full bg-[var(--color-ec-danger-600)]/70" /><span className="size-2 rounded-full bg-[var(--color-ec-warning-600)]/70" /><span className="size-2 rounded-full bg-[var(--color-ec-success-600)]/70" /><div className="ml-3 flex h-6 min-w-0 flex-1 items-center rounded-[var(--ec-radius-sm)] bg-[var(--color-ec-surface)] px-2 text-[8px] text-[var(--color-ec-text-muted)]">electrocms.local / preview</div></div><div className="grid h-[calc(100%-36px)] place-items-center bg-[var(--color-ec-surface-subtle)]"><div className="max-w-sm text-center"><span className="mx-auto mb-3 grid size-12 place-items-center rounded-[var(--ec-radius-xl)] bg-[var(--color-ec-chrome)] text-[var(--color-ec-accent)]"><Icon name="preview" size={20} /></span><strong className="block text-[13px] font-semibold text-[var(--color-ec-text)]">Live site preview</strong><span className="mt-1 block text-[10px] leading-5 text-[var(--color-ec-text-muted)]">This surface uses the same project, breakpoints and theme configuration as the builder.</span></div></div></div><aside className="min-h-0 overflow-y-auto rounded-[var(--ec-radius-lg)]"><ProjectThemeControls scope="frontend" /></aside></div></div></section>; }
function BackendWorkspace() { const menu = ['Overview', 'Products', 'Orders', 'Customers', 'Inventory', 'Content', 'Reports']; return <section className="min-h-0 flex-1 overflow-y-auto bg-[var(--color-ec-app)] p-3 md:p-5" aria-label="Backend workspace"><div className="mx-auto max-w-[1440px]"><div className="mb-3"><span className="text-[8px] font-semibold uppercase tracking-[.18em] text-[var(--color-ec-accent)]">Administrative experience</span><h2 className="mt-0.5 text-lg font-semibold tracking-[-.02em] text-[var(--color-ec-text)]">Backend workspace</h2></div><div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="grid min-h-[650px] overflow-hidden rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] shadow-[var(--ec-shadow-panel)] md:grid-cols-[190px_minmax(0,1fr)]"><aside className="border-r border-white/[.06] bg-[var(--color-ec-chrome)] p-3 text-white"><strong className="mb-4 flex items-center gap-2 text-[11px]"><span className="grid size-7 place-items-center rounded-[var(--ec-radius-md)] bg-[var(--color-ec-chrome-raised)] text-[var(--color-ec-accent)]"><Icon name="bolt" size={14} /></span>Workspace Admin</strong><div className="space-y-1">{menu.map((item, index) => <button type="button" key={item} className={`flex h-8 w-full items-center gap-2 rounded-[var(--ec-radius-md)] px-2 text-[9px] font-medium transition-colors ${index === 0 ? 'bg-[var(--color-ec-accent-soft)] text-[var(--color-ec-accent)]' : 'text-white/45 hover:bg-white/[.06] hover:text-white'}`}><Icon name={item === 'Overview' ? 'grid' : item === 'Products' ? 'content' : item === 'Customers' ? 'users' : 'list'} size={12} />{item}</button>)}</div></aside><main className="min-w-0 bg-[var(--color-ec-surface-subtle)] p-4"><span className="text-[8px] text-[var(--color-ec-text-muted)]">Workspace / Overview</span><strong className="mt-1 block text-[15px] font-semibold text-[var(--color-ec-text)]">Operations dashboard</strong><div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{['Revenue', 'Orders', 'Average order', 'Low stock'].map((item) => <article className="rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-3" key={item}><span className="text-[8px] font-medium text-[var(--color-ec-text-muted)]">{item}</span><strong className="mt-2 block text-lg font-semibold tracking-tight text-[var(--color-ec-text)]">—</strong></article>)}</div><div className="mt-3 rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)]"><div className="flex h-10 items-center justify-between border-b border-[var(--color-ec-border)] px-3"><strong className="text-[10px] text-[var(--color-ec-text)]">Recent records</strong><button className="text-[9px] font-semibold text-[var(--color-ec-accent)]" type="button">View all</button></div>{['Primary table', 'Filters', 'Saved views', 'Bulk actions'].map((item) => <button key={item} type="button" className="flex h-10 w-full items-center justify-between border-b border-[var(--color-ec-border)] px-3 text-[9px] text-[var(--color-ec-text-muted)] last:border-0 hover:bg-[var(--color-ec-surface-muted)]"><span>{item}</span><Icon name="expand" size={11} /></button>)}</div></main></div><aside><ProjectThemeControls scope="backend" /></aside></div></div></section>; }
function ExportWorkspace() { const targets = [{ label: 'Local', detail: 'Portable local package', icon: 'local' as const }, { label: 'React', detail: 'Vite / React deployment', icon: 'code' as const }, { label: 'LAMP', detail: 'PHP + MySQL/MariaDB', icon: 'backend' as const }, { label: 'WordPress', detail: 'Theme + companion plugin', icon: 'blocks' as const }]; return <section className="min-h-0 flex-1 overflow-y-auto bg-[var(--color-ec-app)] p-4 md:p-5" aria-label="Export workspace"><header className="mx-auto max-w-[1200px] border-b border-[var(--color-ec-border)] pb-4"><span className="text-[8px] font-semibold uppercase tracking-[.18em] text-[var(--color-ec-accent)]">Publishing center</span><h2 className="mt-1 text-xl font-semibold tracking-[-.03em] text-[var(--color-ec-text)]">Export workspace</h2><p className="mt-1 text-[10px] text-[var(--color-ec-text-muted)]">Package the same project for different production environments.</p></header><div className="mx-auto mt-5 grid max-w-[1200px] gap-3 sm:grid-cols-2 xl:grid-cols-4">{targets.map((target) => <article key={target.label} className="rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-4 shadow-[var(--ec-shadow-panel)]"><span className="grid size-10 place-items-center rounded-[var(--ec-radius-lg)] bg-[var(--color-ec-accent-soft)] text-[var(--color-ec-accent)]"><Icon name={target.icon} size={17} /></span><strong className="mt-4 block text-[11px] font-semibold text-[var(--color-ec-text)]">{target.label}</strong><span className="mt-1 block text-[9px] leading-4 text-[var(--color-ec-text-muted)]">{target.detail}</span><button type="button" className={`${quietButton} mt-4 w-full`} disabled>Configure</button></article>)}</div></section>; }
...
export function ProductionStudio({ workspaceId, compactLayout, navigationOpen, onCloseNavigation, onNavigate }: ProductionStudioProps) {
  ...
  return <main className="production-studio flex min-h-0 flex-1 overflow-hidden bg-[var(--color-ec-app)] text-[var(--color-ec-text)]" id="workspace-main" tabIndex={-1} data-workspace={workspaceId} data-compact={compactLayout ? 'true' : 'false'} data-navigation-position={preferences.navigationPosition} data-navigation-collapsed={collapsed ? 'true' : 'false'}><h1 className="sr-only">{workspaceLabel} workspace</h1>{!compactLayout && preferences.navigationPosition === 'left' ? rail : null}<div className="flex min-w-0 flex-1 flex-col"><div className="flex h-10 shrink-0 items-center justify-between gap-3 border-b border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] px-3"><div className="flex min-w-0 items-center gap-2"><span className="text-[8px] font-semibold uppercase tracking-[.18em] text-[var(--color-ec-text-muted)]">{workspaceId === 'editor' ? 'Studio' : 'Workspace'}</span><span className="h-3 w-px bg-[var(--color-ec-border)]" /><strong className="truncate text-[10px] font-semibold text-[var(--color-ec-text)]">{workspaceId === 'editor' ? activeDefinition?.label ?? 'Builder' : workspaceLabel}</strong></div><div className="flex items-center gap-1"><button type="button" className={`${quietButton} hidden md:inline-flex`}><Icon name="command" size={12} />Commands</button><button type="button" className={`${quietButton} hidden lg:inline-flex`}><Icon name="users" size={12} />Share</button></div></div><div className="flex min-h-0 flex-1">{content}</div></div>{!compactLayout && preferences.navigationPosition === 'right' ? rail : null}{compactLayout && navigationOpen ? <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-[2px]" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) onCloseNavigation(); }}><div className="h-full w-[min(86vw,260px)] shadow-2xl" role="dialog" aria-modal="true" aria-label="Workspace navigation">{rail}</div></div> : null}</main>;
}

...
const modules: readonly StudioModuleDefinition[] = [
  ...
  { id: 'themes', label: 'Site Design', description: 'Frontend and backend design systems', icon: 'theme' },
...
const quietButton = 'ec-control ec-focus-ring ec-action inline-flex h-8 items-center justify-center gap-1.5 px-2.5 text-[10px] font-semibold text-[var(--color-ec-text-muted)]';
...
function StudioRail({ compactLayout, workspaceId, activeModule, settingsOpen, onSettingsOpenChange, onNavigate, onSelectModule, onClose }: {
  ...
  const { preferences, setNavigationPosition, setNavigationWidth, setNavigationCollapsed, moveWorkspace, reset } = useWorkspacePreferences();
  const collapsed = !compactLayout && preferences.navigationCollapsed;
  const displayMode = collapsed ? 'icons' : 'both';
  const showLabels = !collapsed;
  const railButtonClass = 'ec-focus-ring ec-action studio-nav-button group relative flex h-9 w-full items-center gap-2 rounded-[var(--ec-radius-md)] px-2 text-left text-[11px] font-medium text-white/55 hover:bg-white/[.06] hover:text-white data-[active=true]:text-[var(--color-ec-accent)]';
  ...
  return (
    <aside
      className={`workspace-navigation studio-rail relative flex h-full shrink-0 flex-col p-2 text-white ${collapsed ? 'w-[60px]' : compactLayout ? 'w-full' : 'min-w-[208px] max-w-[344px]'}`}
    ...
    >
      <div className="mb-2 flex h-10 items-center justify-between gap-2 px-1">
        <div className="flex min-w-0 items-center gap-2">
          <div className="grid size-8 shrink-0 place-items-center rounded-[var(--ec-radius-md)] border border-[rgb(49_198_188_/_0.16)] bg-[var(--color-ec-chrome-raised)] text-[var(--color-ec-accent)]"><Icon name="bolt" size={16} /></div>
          {showLabels ? <div className="min-w-0"><strong className="block truncate text-[12px] font-semibold tracking-[-.01em] text-white">ElectroCMS</strong><span className="block text-[9px] font-semibold uppercase tracking-[.14em] text-white/35">Bento Density</span></div> : null}
        </div>
        <button className="ec-focus-ring ec-action grid size-8 shrink-0 place-items-center rounded-[var(--ec-radius-md)] text-white/40 hover:bg-white/[.06] hover:text-white" type="button" aria-label={compactLayout ? 'Close navigation' : collapsed ? 'Expand navigation' : 'Collapse navigation'} onClick={compactLayout ? onClose : () => setNavigationCollapsed(!collapsed)}><Icon name={compactLayout ? 'close' : collapsed ? 'expand' : 'collapse'} size={15} /></button>
      </div>
      ...
      <nav className="space-y-1" aria-label="Primary workspaces">
        {preferences.workspaceOrder.map((workspaceIdFromOrder) => {
          ...
          return <button key={workspace.id} type="button" className={railButtonClass} data-active={workspaceId === workspace.id ? 'true' : 'false'} aria-label={workspace.label} title={workspace.label} onClick={() => { onNavigate(workspace.id); if (compactLayout) onClose(); }}>{workspaceId === workspace.id ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-[var(--color-ec-accent)]" aria-hidden="true" /> : null}<span className="grid size-6 shrink-0 place-items-center"><Icon name={workspace.icon} size={15} /></span>{showLabels ? <span className="min-w-0 flex-1 truncate">{workspace.label}</span> : null}</button>;
        })}
      </nav>

      <div className="my-2 h-px bg-white/[.07]" />
      {showLabels ? <span className="mb-1 px-2 text-[9px] font-semibold uppercase tracking-[.15em] text-white/30">Build</span> : null}
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5 [scrollbar-width:thin] [scrollbar-color:rgba(255,255,255,.12)_transparent]" aria-label="Studio modules">
        {modules.map((module) => <button key={module.id} type="button" className={railButtonClass} data-active={workspaceId === 'editor' && activeModule === module.id ? 'true' : 'false'} aria-label={module.label} title={`${module.label} — ${module.description}`} onClick={() => { onSelectModule(module.id); if (compactLayout) onClose(); }}>{workspaceId === 'editor' && activeModule === module.id ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-[var(--color-ec-accent)]" aria-hidden="true" /> : null}<span className="grid size-6 shrink-0 place-items-center"><Icon name={module.icon} size={15} /></span>{showLabels ? <span className="min-w-0 flex-1 truncate">{module.label}</span> : null}</button>)}
      </nav>

      <details className="group/settings relative mt-2 border-t border-white/[.07] pt-2" open={settingsOpen} onToggle={(event) => onSettingsOpenChange(event.currentTarget.open)}>
        <summary className="ec-focus-ring ec-action flex h-9 cursor-pointer list-none items-center gap-2 rounded-[var(--ec-radius-md)] px-2 text-[10px] font-medium text-white/45 hover:bg-white/[.06] hover:text-white [&::-webkit-details-marker]:hidden"><span className="grid size-6 place-items-center"><Icon name="settings" size={14} /></span>{showLabels ? <span>Workspace layout</span> : <span className="sr-only">Workspace layout</span>}</summary>
        <div className={`studio-settings-popover absolute bottom-0 z-50 w-[264px] rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-3 text-[var(--color-ec-text)] shadow-[var(--ec-shadow-float)] ${preferences.navigationPosition === 'right' ? 'right-full mr-2' : 'left-full ml-2'}`}>
          <div className="mb-3"><span className="text-[9px] font-bold uppercase tracking-[.12em] text-[var(--color-ec-accent)]">Bento Density</span><strong className="mt-1 block text-[11px]">Layout preferences</strong><p className="mt-1 text-[10px] leading-4 text-[var(--color-ec-text-muted)]">The interface theme and density are fixed. You can still position, resize and reorder the workspace rail.</p></div>
          <label className="text-[10px] font-medium text-[var(--color-ec-text-muted)]"><span className="mb-1 block">Navigation position</span><select className="ec-control h-8 w-full px-2 text-[10px]" aria-label="Navigation position" value={preferences.navigationPosition} onChange={(event) => setNavigationPosition(event.target.value as 'left' | 'right')}><option value="left">Left</option><option value="right">Right</option></select></label>
          <fieldset className="mt-3 border-t border-[var(--color-ec-border)] pt-3"><legend className="mb-1 text-[9px] font-semibold uppercase tracking-[.12em] text-[var(--color-ec-text-muted)]">Workspace order</legend><div className="space-y-1">{preferences.workspaceOrder.map((orderedWorkspace, index) => { const workspace = primaryWorkspaces.find((item) => item.id === orderedWorkspace); if (!workspace) return null; return <div key={orderedWorkspace} className="flex min-h-8 items-center justify-between rounded-[var(--ec-radius-sm)] px-1.5 text-[10px] hover:bg-[var(--color-ec-surface-muted)]"><span className="flex items-center gap-2"><Icon name={workspace.icon} size={11} />{workspace.label}</span><span className="flex gap-0.5"><button className="ec-focus-ring ec-action grid size-7 place-items-center rounded text-[var(--color-ec-text-muted)] hover:bg-[var(--color-ec-surface-subtle)] hover:text-[var(--color-ec-text)] disabled:opacity-25" type="button" aria-label={`Move ${workspace.label} up`} disabled={index === 0} onClick={() => moveWorkspace(orderedWorkspace, -1)}><Icon name="arrow-up" size={12} /></button><button className="ec-focus-ring ec-action grid size-7 place-items-center rounded text-[var(--color-ec-text-muted)] hover:bg-[var(--color-ec-surface-subtle)] hover:text-[var(--color-ec-text)] disabled:opacity-25" type="button" aria-label={`Move ${workspace.label} down`} disabled={index === preferences.workspaceOrder.length - 1} onClick={() => moveWorkspace(orderedWorkspace, 1)}><Icon name="arrow-down" size={12} /></button></span></div>; })}</div></fieldset>
          <button className="ec-control ec-focus-ring ec-action mt-3 h-8 w-full text-[10px] font-semibold" type="button" onClick={reset}>Reset workspace layout</button>
        </div>
      </details>

      {!compactLayout && !collapsed ? <div className={`group/resizer absolute inset-y-0 z-20 w-3 touch-none cursor-col-resize outline-none ${preferences.navigationPosition === 'left' ? '-right-1.5' : '-left-1.5'}`} role="separator" aria-label="Resize navigation" aria-orientation="vertical" aria-valuemin={MIN_NAVIGATION_WIDTH} aria-valuemax={MAX_NAVIGATION_WIDTH} aria-valuenow={preferences.navigationWidth} aria-valuetext={`${preferences.navigationWidth} pixels`} tabIndex={0} onPointerDown={handleResizeStart} onKeyDown={handleResizeKeyDown}><span className="absolute inset-y-5 left-1/2 w-px -translate-x-1/2 rounded-full bg-transparent transition-colors group-hover/resizer:bg-[var(--color-ec-accent)] group-focus-visible/resizer:bg-[var(--color-ec-accent)]" aria-hidden="true" /></div> : null}
    </aside>
  );
}
...
function WidgetLibrary({ onInsert }: { onInsert(definition: WidgetDefinition): void }) {
  ...
  return (
    <aside className="studio-library flex min-h-0 w-[252px] shrink-0 flex-col" aria-label="Insert library">
      <div className="flex h-12 items-center justify-between border-b border-[var(--color-ec-border)] px-3"><div><span className="block text-[9px] font-semibold uppercase tracking-[.14em] text-[var(--color-ec-accent)]">Insert</span><strong className="text-[11px] font-semibold text-[var(--color-ec-text)]">Element library</strong></div><span className="grid size-7 place-items-center rounded-[var(--ec-radius-md)] bg-[var(--color-ec-accent-soft)] text-[var(--color-ec-accent)]"><Icon name="blocks" size={14} /></span></div>
      <div className="p-2.5"><label className="ec-control flex h-9 items-center gap-2 px-2.5 text-[var(--color-ec-text-muted)] focus-within:border-[var(--color-ec-accent)] focus-within:shadow-[var(--ec-focus-ring)]"><Icon name="search" size={13} /><input className="min-w-0 flex-1 bg-transparent text-[10px] text-[var(--color-ec-text)] outline-none placeholder:text-[var(--color-ec-text-faint)]" aria-label="Search elements" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search elements" /></label></div>
      <div className="flex gap-1 overflow-x-auto border-b border-[var(--color-ec-border)] px-2.5 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Element categories"><button className="ec-focus-ring ec-action h-7 shrink-0 rounded-[var(--ec-radius-sm)] px-2 text-[9px] font-semibold text-[var(--color-ec-text-muted)] hover:bg-[var(--color-ec-surface-muted)] data-[active=true]:bg-[var(--color-ec-accent-soft)] data-[active=true]:text-[var(--color-ec-accent)]" type="button" data-active={activeCategory === 'all'} onClick={() => setActiveCategory('all')}>All</button>{categories.map((category) => <button className="ec-focus-ring ec-action h-7 shrink-0 rounded-[var(--ec-radius-sm)] px-2 text-[9px] font-semibold text-[var(--color-ec-text-muted)] hover:bg-[var(--color-ec-surface-muted)] data-[active=true]:bg-[var(--color-ec-accent-soft)] data-[active=true]:text-[var(--color-ec-accent)]" key={category} type="button" data-active={activeCategory === category} onClick={() => setActiveCategory(category)}>{categoryLabels[category] ?? category}</button>)}</div>
      <div className="min-h-0 flex-1 overflow-y-auto p-2.5 [scrollbar-width:thin]">{categories.map((category) => { const group = visible.filter((definition) => definition.metadata.category === category); if (group.length === 0) return null; return <section className="mb-4" key={category}><div className="mb-1.5 flex items-center justify-between px-0.5 text-[9px] font-semibold uppercase tracking-[.12em] text-[var(--color-ec-text-muted)]"><span className="flex items-center gap-1.5"><Icon name={categoryIcons[category] ?? 'blocks'} size={11} />{categoryLabels[category] ?? category}</span><small className="font-medium tabular-nums">{group.length}</small></div><div className="grid grid-cols-2 gap-1.5">{group.map((definition) => <button key={`${definition.type}@${definition.version}`} type="button" className="ec-focus-ring ec-action group flex min-h-16 flex-col items-start justify-between rounded-[var(--ec-radius-md)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface-subtle)] p-2 text-left hover:border-[rgb(49_198_188_/_0.28)] hover:bg-[var(--color-ec-surface-muted)]" aria-label={`Add ${definition.metadata.name} from element library`} title={`${definition.metadata.name} — ${definition.metadata.description}`} onClick={() => onInsert(definition)}><span className="grid size-7 place-items-center rounded-[var(--ec-radius-sm)] bg-[var(--color-ec-surface-muted)] text-[var(--color-ec-text-muted)] transition-colors group-hover:bg-[var(--color-ec-accent-soft)] group-hover:text-[var(--color-ec-accent)]"><Icon name={categoryIcons[definition.metadata.category] ?? 'blocks'} size={13} /></span><span className="mt-2 line-clamp-1 text-[9px] font-semibold text-[var(--color-ec-text)]">{definition.metadata.name}</span></button>)}</div></section>; })}{visible.length === 0 ? <div className="grid min-h-36 place-items-center rounded-[var(--ec-radius-lg)] border border-dashed border-[var(--color-ec-border)] bg-[var(--color-ec-surface-subtle)] p-5 text-center"><div><span className="mx-auto mb-2 grid size-9 place-items-center rounded-full bg-[var(--color-ec-surface)] text-[var(--color-ec-text-muted)]"><Icon name="search" size={16} /></span><strong className="block text-[10px] text-[var(--color-ec-text)]">No elements found</strong><span className="mt-1 block text-[9px] text-[var(--color-ec-text-muted)]">Try another search or category.</span></div></div> : null}</div>
    </aside>
  );
}
...
function BuilderWorkspace() {
  ...
  return <div className="studio-builder-workspace flex min-h-0 flex-1 overflow-hidden"><WidgetLibrary onInsert={(definition) => actions.insertWidget(definition.type)} /><div className="studio-editor-region flex min-w-0 flex-1 flex-col"><div className="studio-canvas-context flex h-10 shrink-0 items-center justify-between rounded-[var(--ec-radius-md)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] px-3"><div className="flex min-w-0 items-center gap-2 text-[9px] text-[var(--color-ec-text-muted)]"><Icon name="editor" size={12} /><span className="truncate">{document.name}</span><span>/</span><strong className="font-semibold text-[var(--color-ec-text)]">Canvas</strong></div><span className="hidden items-center gap-1.5 rounded-full bg-[var(--color-ec-surface-muted)] px-2 py-1 text-[9px] font-semibold text-[var(--color-ec-text-muted)] md:inline-flex"><Icon name="grid" size={10} />{breakpoint.label} · {breakpoint.width}px</span></div><EditorCanvas document={document} breakpointId={session.activeBreakpointId} breakpoints={session.project.breakpoints} viewportWidth={breakpoint.width} zoom={session.zoom} actions={actions} /></div></div>;
}
...
export function ProductionStudio({ workspaceId, editorModuleId, compactLayout, navigationOpen, onCloseNavigation, onNavigate, onNavigateEditorModule }: ProductionStudioProps) {
  ...
  return (
    <main className="production-studio flex min-h-0 flex-1 overflow-hidden text-[var(--color-ec-text)]" id="workspace-main" tabIndex={-1} data-workspace={workspaceId} data-editor-module={editorModuleId} data-compact={compactLayout ? 'true' : 'false'} data-navigation-position={preferences.navigationPosition} data-navigation-collapsed={collapsed ? 'true' : 'false'}>
      <h1 className="sr-only">{workspaceLabel} workspace</h1>
      {!compactLayout && preferences.navigationPosition === 'left' ? rail : null}
      <div className="studio-workspace-frame flex min-w-0 flex-1 flex-col">
        <div className="studio-context-bar flex shrink-0 items-center justify-between gap-3 px-3"><div className="flex min-w-0 items-center gap-2"><span className="grid size-6 place-items-center rounded-[var(--ec-radius-sm)] bg-[var(--color-ec-surface-muted)] text-[var(--color-ec-accent)]"><Icon name={workspaceId === 'editor' ? activeDefinition?.icon ?? 'editor' : primaryWorkspaces.find((item) => item.id === workspaceId)?.icon ?? 'editor'} size={12} /></span><div className="min-w-0"><span className="block text-[9px] font-semibold uppercase tracking-[.12em] text-[var(--color-ec-text-muted)]">{workspaceId === 'editor' ? 'Studio' : 'Workspace'}</span><strong className="block truncate text-[10px] font-semibold text-[var(--color-ec-text)]">{workspaceId === 'editor' ? activeDefinition?.label ?? 'Builder' : workspaceLabel}</strong></div></div><button type="button" className={`${quietButton} hidden md:inline-flex`} aria-label="Open command palette" title="Command palette · Ctrl/⌘ K" onClick={() => setCommandOpen(true)}><Icon name="command" size={12} />Commands</button></div>
        <div className="flex min-h-0 flex-1">{content}</div>
      </div>
      {!compactLayout && preferences.navigationPosition === 'right' ? rail : null}
      {compactLayout && navigationOpen ? <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-[3px]" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) onCloseNavigation(); }}><div className="h-full w-[min(88vw,300px)]" role="dialog" aria-modal="true" aria-label="Workspace navigation">{rail}</div></div> : null}

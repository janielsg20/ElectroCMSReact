...
function StudioRail({
  ...
  return (
    <aside className={`workspace-navigation studio-rail flex h-full shrink-0 flex-col border-slate-800/80 bg-[#09111f] p-2 text-white shadow-xl shadow-slate-950/5 ${collapsed ? 'w-[60px]' : 'w-[224px]'} ${preferences.navigationPosition === 'right' ? 'border-l' : 'border-r'}`} aria-label="Workspace navigation" data-position={preferences.navigationPosition} data-collapsed={collapsed ? 'true' : 'false'} data-display-mode={displayMode}>
      <div className="mb-2 flex h-10 items-center justify-between gap-2 px-1">
        <div className="flex min-w-0 items-center gap-2"><div className="grid size-8 shrink-0 place-items-center rounded-[10px] bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-950/30"><Icon name="bolt" size={16} /></div>{showLabels ? <div className="min-w-0"><strong className="block truncate text-[12px] font-semibold tracking-tight text-white">ElectroCMS</strong><span className="block text-[9px] font-medium uppercase tracking-[.18em] text-slate-500">Studio</span></div> : null}</div>
        {compactLayout ? <button className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-white/8 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/40" type="button" aria-label="Close navigation" onClick={onClose}><Icon name="close" size={15} /></button> : <button className="grid size-8 shrink-0 place-items-center rounded-lg text-slate-500 hover:bg-white/8 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/40" type="button" aria-label={collapsed ? 'Expand navigation' : 'Collapse navigation'} onClick={() => setNavigationCollapsed(!collapsed)}><Icon name={collapsed ? 'expand' : 'collapse'} size={15} /></button>}
      </div>
      ...
      <nav className="space-y-1" aria-label="Primary workspaces">
        {preferences.workspaceOrder.map((workspaceIdFromOrder) => {
          ...
          return <button key={workspace.id} type="button" className={railButtonClass} data-active={workspaceId === workspace.id ? 'true' : 'false'} aria-label={workspace.label} title={workspace.label} onClick={() => { onNavigate(workspace.id); if (compactLayout) onClose(); }}>{workspaceId === workspace.id ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-blue-400" aria-hidden="true" /> : null}{showIcons ? <span className="grid size-6 shrink-0 place-items-center text-slate-400 group-data-[active=true]:text-blue-300"><Icon name={workspace.icon} size={16} /></span> : null}{showLabels ? <span className="min-w-0 flex-1 truncate">{workspace.label}</span> : null}</button>;
        })}
      </nav>
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5 [scrollbar-width:thin] [scrollbar-color:#334155_transparent]" aria-label="Studio modules">
        {modules.map((module) => <button key={module.id} type="button" className={railButtonClass} data-active={workspaceId === 'editor' && activeModule === module.id ? 'true' : 'false'} aria-label={module.label} title={module.label} onClick={() => { onSelectModule(module.id); if (compactLayout) onClose(); }}>{workspaceId === 'editor' && activeModule === module.id ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-blue-400" aria-hidden="true" /> : null}{showIcons ? <span className="grid size-6 shrink-0 place-items-center text-slate-500 group-data-[active=true]:text-blue-300"><Icon name={module.icon} size={16} /></span> : null}{showLabels ? <span className="min-w-0 flex-1 truncate">{module.label}</span> : null}</button>)}
      </nav>

      <details className="group/settings relative mt-2 border-t border-white/8 pt-2" open={settingsOpen} onToggle={(event) => onSettingsOpenChange(event.currentTarget.open)}>
        <summary className="flex h-9 cursor-pointer list-none items-center gap-2 rounded-lg px-2 text-[11px] font-medium text-slate-500 transition hover:bg-white/7 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/40 [&::-webkit-details-marker]:hidden"><span className="grid size-6 place-items-center"><Icon name="settings" size={15} /></span>{showLabels ? <span>Workspace settings</span> : <span className="sr-only">Workspace settings</span>}</summary>
...
function WidgetLibrary({ onInsert }: { onInsert(definition: WidgetDefinition): void }) {
  ...
  return <aside className="studio-library flex min-h-0 w-[272px] shrink-0 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950" aria-label="Insert library">
...
function BuilderWorkspace() {
  ...
  return <div className="studio-builder-workspace flex min-h-0 flex-1 overflow-hidden"><WidgetLibrary onInsert={(definition) => actions.insertWidget(definition.type)} /><div className="studio-editor-region flex min-w-0 flex-1 flex-col bg-slate-100/80 dark:bg-slate-900"><div className="flex h-10 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-3 dark:border-slate-800 dark:bg-slate-950"><div className="flex min-w-0 items-center gap-1.5 text-[10px] text-slate-400"><span className="truncate">{document.name}</span><span>/</span><strong className="font-semibold text-slate-600 dark:text-slate-300">Canvas</strong></div><div className="flex items-center gap-1"><button className={quietButton} type="button"><Icon name="grid" size={13} />Grid</button><button className={quietButton} type="button"><Icon name="layers" size={13} />Layers</button><button className={quietButton} type="button"><Icon name="command" size={13} />Commands</button></div></div><EditorCanvas document={document} breakpointId={session.activeBreakpointId} breakpoints={session.project.breakpoints} viewportWidth={breakpoint.width} zoom={session.zoom} actions={actions} /></div></div>;
}
...
function FeatureCard({ icon, title, description, items }: FeatureGroup) {
  return <article className="group rounded-xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/[.02] transition hover:border-slate-300 hover:shadow-md hover:shadow-slate-900/[.04] dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-[10px] bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-300"><Icon name={icon} size={17} /></span><div className="min-w-0 flex-1"><strong className="block text-[12px] font-semibold text-slate-900 dark:text-white">{title}</strong><p className="mt-0.5 text-[10px] leading-4 text-slate-500 dark:text-slate-400">{description}</p></div></div><div className="mt-4 grid gap-1 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">{items.map((item) => <button key={item} type="button" className="flex h-8 items-center justify-between rounded-lg px-2 text-left text-[10px] font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/20 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"><span className="flex items-center gap-2"><span className="size-1 rounded-full bg-slate-300 dark:bg-slate-600" />{item}</span><Icon name="expand" size={11} /></button>)}</div></article>;
}
...
function ModuleWorkspace({ module }: { module: Exclude<StudioModuleId, 'builder' | 'themes'> }) {
  ...
  return <section className="min-h-0 flex-1 overflow-y-auto bg-slate-50 p-4 md:p-5 dark:bg-slate-950" aria-label={`${definition?.label ?? module} workspace`}><header className="mx-auto flex max-w-[1280px] flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between dark:border-slate-800"><div><span className="text-[9px] font-semibold uppercase tracking-[.16em] text-blue-600 dark:text-blue-400">ElectroCMS Studio</span><h2 className="mt-1 text-xl font-semibold tracking-[-.03em] text-slate-950 dark:text-white">{definition?.label ?? module}</h2><p className="mt-1 max-w-2xl text-[11px] leading-5 text-slate-500 dark:text-slate-400">{definition?.description}</p></div><button type="button" className={quietButton}><Icon name="more" size={14} />Options</button></header><div className="mx-auto mt-5 grid max-w-[1280px] gap-3 lg:grid-cols-3">{moduleFeatures[module].map((group) => <FeatureCard key={group.title} {...group} />)}</div></section>;
}
...
export function ProductionStudio({ workspaceId, compactLayout, navigationOpen, onCloseNavigation, onNavigate }: ProductionStudioProps) {
  ...
  return <main className="production-studio flex min-h-0 flex-1 overflow-hidden bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100" id="workspace-main" tabIndex={-1} data-workspace={workspaceId} data-compact={compactLayout ? 'true' : 'false'} data-navigation-position={preferences.navigationPosition} data-navigation-collapsed={collapsed ? 'true' : 'false'}><h1 className="sr-only">{workspaceLabel} workspace</h1>{!compactLayout && preferences.navigationPosition === 'left' ? rail : null}<div className="flex min-w-0 flex-1 flex-col"><div className="flex h-11 shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white px-3 dark:border-slate-800 dark:bg-slate-950"><div className="flex min-w-0 items-center gap-2"><span className="text-[9px] font-semibold uppercase tracking-[.16em] text-slate-400">{workspaceId === 'editor' ? 'Studio' : 'Workspace'}</span><span className="h-3 w-px bg-slate-200 dark:bg-slate-800" /><strong className="truncate text-[11px] font-semibold text-slate-700 dark:text-slate-200">{workspaceId === 'editor' ? activeDefinition?.label ?? 'Builder' : workspaceLabel}</strong></div><div className="flex items-center gap-1"><button type="button" className={`${quietButton} hidden md:inline-flex`}><Icon name="command" size={13} />Command palette</button><button type="button" className={`${quietButton} hidden lg:inline-flex`}><Icon name="users" size={13} />Share</button><button type="button" className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-blue-600 px-3 text-[11px] font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30" disabled={workspaceId === 'export'}><Icon name="export" size={13} />Publish</button></div></div><div className="flex min-h-0 flex-1">{content}</div></div>{!compactLayout && preferences.navigationPosition === 'right' ? rail : null}{compactLayout && navigationOpen ? <div className="fixed inset-0 z-50 bg-slate-950/45 backdrop-blur-[2px]" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) onCloseNavigation(); }}><div className="h-full w-[min(86vw,260px)] shadow-2xl" role="dialog" aria-modal="true" aria-label="Workspace navigation">{rail}</div></div> : null}</main>;
}

...
const modules: readonly StudioModuleDefinition[] = [
  { id: 'builder', label: 'Builder', description: 'Visual page builder', icon: 'editor', state: 'ready' },
  { id: 'pages', label: 'Pages', description: 'Pages and templates', icon: 'pages', state: 'planned' },
  { id: 'content', label: 'Content', description: 'Models and records', icon: 'database', state: 'planned' },
  { id: 'queries', label: 'Queries', description: 'Dynamic data queries', icon: 'query', state: 'planned' },
  { id: 'forms', label: 'Forms', description: 'Forms and actions', icon: 'form', state: 'planned' },
  { id: 'filters', label: 'Filters', description: 'Smart filtering', icon: 'filter', state: 'planned' },
  { id: 'media', label: 'Media', description: 'Assets and files', icon: 'media', state: 'planned' },
  { id: 'themes', label: 'Themes', description: 'Design systems', icon: 'theme', state: 'ready' },
  { id: 'users', label: 'Roles', description: 'Users and access', icon: 'users', state: 'planned' },
  { id: 'blueprints', label: 'Blueprints', description: 'Project starters', icon: 'blueprint', state: 'planned' },
  { id: 'settings', label: 'Settings', description: 'Project configuration', icon: 'settings', state: 'partial' },
];

const primaryWorkspaces: readonly { id: WorkspaceId; label: string; icon: IconName; state: ImplementationState }[] = [
...
function ImplementationDot({ state, label }: { state: ImplementationState; label?: string }) {
  if (state === 'ready') return null;
  return <span className="size-1.5 shrink-0 rounded-full bg-red-500 shadow-[0_0_0_1px_rgba(239,68,68,.12)]" title={label ?? 'Not fully implemented yet'} aria-label={label ?? 'Not fully implemented yet'} />;
}
        ...
        {preferences.workspaceOrder.map((workspaceIdFromOrder) => {
          ...
          return (
            ...
            >
              ...
              <ImplementationDot state={workspace.state} />
            </button>
          );
        })}
      </nav>
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5 [scrollbar-width:thin] [scrollbar-color:#334155_transparent]" aria-label="Studio modules">
        {modules.map((module) => (
          ...
          >
            ...
            <ImplementationDot state={module.state} />
          </button>
        ))}
      </nav>

      <details
        className="group/settings relative mt-2 border-t border-white/8 pt-2"
        open={settingsOpen}
        onToggle={(event) => onSettingsOpenChange(event.currentTarget.open)}
      >
                ...
                {group.map((definition) => {
                  ...
                  return (
                    <button key={`${definition.type}@${definition.version}`} type="button" className="group relative flex min-h-16 flex-col items-start justify-between rounded-lg border border-slate-200 bg-white p-2 text-left transition hover:-translate-y-px hover:border-blue-300 hover:shadow-sm hover:shadow-blue-900/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/20 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-700" aria-label={`Add ${definition.metadata.name} from element library`} title={`${definition.metadata.name} — ${definition.metadata.description}`} onClick={() => onInsert(definition)}>
                      ...
                      {ready ? null : <span className="absolute right-2 top-2 size-1.5 rounded-full bg-red-500" title="Not fully implemented yet" aria-label="Not fully implemented yet" />}
                    </button>
                  );
                })}
              </div>
    ...
    <div className="studio-builder-workspace flex min-h-0 flex-1 overflow-hidden">
      ...
      <div className="studio-editor-region flex min-w-0 flex-1 flex-col bg-slate-100/80 dark:bg-slate-900">
        <div className="flex h-10 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-3 dark:border-slate-800 dark:bg-slate-950">
          ...
          <div className="flex items-center gap-1"><button className={quietButton} type="button"><Icon name="grid" size={13} />Grid<ImplementationDot state="planned" /></button><button className={quietButton} type="button"><Icon name="layers" size={13} />Layers<ImplementationDot state="planned" /></button><button className={quietButton} type="button"><Icon name="command" size={13} />Commands<ImplementationDot state="planned" /></button></div>
        </div>
...
function FeatureCard({ icon, title, description, items }: FeatureGroup) {
  return (
    <article className="group rounded-xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/[.02] transition hover:border-slate-300 hover:shadow-md hover:shadow-slate-900/[.04] dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700">
      <div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-[10px] bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-300"><Icon name={icon} size={17} /></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><strong className="block text-[12px] font-semibold text-slate-900 dark:text-white">{title}</strong><ImplementationDot state="planned" /></div><p className="mt-0.5 text-[10px] leading-4 text-slate-500 dark:text-slate-400">{description}</p></div></div>
      <div className="mt-4 grid gap-1 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">{items.map((item) => <button key={item} type="button" className="flex h-8 items-center justify-between rounded-lg px-2 text-left text-[10px] font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/20 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"><span className="flex items-center gap-2"><ImplementationDot state="planned" />{item}</span><Icon name="expand" size={11} /></button>)}</div>
    </article>
  );
}
...
function ModuleWorkspace({ module }: { module: Exclude<StudioModuleId, 'builder' | 'themes'> }) {
  ...
  return (
    <section className="min-h-0 flex-1 overflow-y-auto bg-slate-50 p-4 md:p-5 dark:bg-slate-950" aria-label={`${definition?.label ?? module} workspace`}>
      <header className="mx-auto flex max-w-[1280px] flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between dark:border-slate-800"><div><span className="text-[9px] font-semibold uppercase tracking-[.16em] text-blue-600 dark:text-blue-400">ElectroCMS Studio</span><div className="mt-1 flex items-center gap-2"><h2 className="text-xl font-semibold tracking-[-.03em] text-slate-950 dark:text-white">{definition?.label ?? module}</h2><ImplementationDot state={definition?.state ?? 'planned'} /></div><p className="mt-1 max-w-2xl text-[11px] leading-5 text-slate-500 dark:text-slate-400">{definition?.description}</p></div><button type="button" className={quietButton}><Icon name="more" size={14} />Options<ImplementationDot state="planned" /></button></header>
...
function BackendWorkspace() {
  ...
  return (
    <section className="min-h-0 flex-1 overflow-y-auto bg-slate-100 p-3 md:p-5 dark:bg-slate-950" aria-label="Backend workspace">
      <div className="mx-auto max-w-[1440px]"><div className="mb-3"><span className="text-[9px] font-semibold uppercase tracking-[.16em] text-blue-600 dark:text-blue-400">Administrative experience</span><div className="mt-0.5 flex items-center gap-2"><h2 className="text-lg font-semibold tracking-[-.02em] text-slate-950 dark:text-white">Backend workspace</h2><ImplementationDot state="partial" /></div></div><div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="grid min-h-[650px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl shadow-slate-900/5 md:grid-cols-[190px_minmax(0,1fr)] dark:border-slate-800 dark:bg-slate-900"><aside className="border-r border-slate-200 bg-slate-950 p-3 text-white dark:border-slate-800"><strong className="mb-4 flex items-center gap-2 text-[12px]"><span className="grid size-7 place-items-center rounded-lg bg-blue-600"><Icon name="bolt" size={14} /></span>Northstar Admin</strong><div className="space-y-1">{menu.map((item, index) => <button type="button" key={item} className={`flex h-8 w-full items-center gap-2 rounded-lg px-2 text-[10px] font-medium transition ${index === 0 ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/7 hover:text-white'}`}><Icon name={item === 'Overview' ? 'grid' : item === 'Products' ? 'content' : item === 'Customers' ? 'users' : 'list'} size={13} />{item}<span className="ml-auto"><ImplementationDot state="planned" /></span></button>)}</div></aside><main className="min-w-0 bg-slate-50 p-4 dark:bg-slate-950"><div><span className="text-[9px] text-slate-400">Workspace / Overview</span><strong className="mt-1 block text-[16px] font-semibold text-slate-900 dark:text-white">Operations dashboard</strong></div><div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{['Revenue', 'Orders', 'Average order', 'Low stock'].map((item) => <article className="relative rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900" key={item}><span className="text-[9px] font-medium text-slate-400">{item}</span><strong className="mt-2 block text-lg font-semibold tracking-tight text-slate-900 dark:text-white">—</strong><span className="absolute right-3 top-3"><ImplementationDot state="planned" /></span></article>)}</div><div className="mt-3 rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="flex h-10 items-center justify-between border-b border-slate-100 px-3 dark:border-slate-800"><strong className="text-[11px] text-slate-700 dark:text-slate-200">Recent records</strong><button className="flex items-center gap-1.5 text-[10px] font-semibold text-blue-600 dark:text-blue-400" type="button">View all<ImplementationDot state="planned" /></button></div>{['Primary table', 'Filters', 'Saved views', 'Bulk actions'].map((item) => <button key={item} type="button" className="flex h-10 w-full items-center justify-between border-b border-slate-100 px-3 text-[10px] text-slate-500 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:text-slate-400 dark:hover:bg-slate-800"><span className="flex items-center gap-2"><ImplementationDot state="planned" />{item}</span><Icon name="expand" size={12} /></button>)}</div></main></div><aside><ProjectThemeControls scope="backend" /></aside></div></div>
    </section>
  );
}
...
function ExportWorkspace() {
  ...
  return <section className="min-h-0 flex-1 overflow-y-auto bg-slate-50 p-4 md:p-5 dark:bg-slate-950" aria-label="Export workspace"><header className="mx-auto max-w-[1200px] border-b border-slate-200 pb-4 dark:border-slate-800"><span className="text-[9px] font-semibold uppercase tracking-[.16em] text-blue-600 dark:text-blue-400">Publishing center</span><div className="mt-1 flex items-center gap-2"><h2 className="text-xl font-semibold tracking-[-.03em] text-slate-950 dark:text-white">Export workspace</h2><ImplementationDot state="planned" /></div><p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Package the same project for different production environments.</p></header><div className="mx-auto mt-5 grid max-w-[1200px] gap-3 sm:grid-cols-2 xl:grid-cols-4">{targets.map((target) => <article key={target.label} className="relative rounded-xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/[.02] dark:border-slate-800 dark:bg-slate-900"><span className="absolute right-4 top-4"><ImplementationDot state="planned" /></span><span className="grid size-10 place-items-center rounded-xl bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-300"><Icon name={target.icon} size={18} /></span><strong className="mt-4 block text-[12px] font-semibold text-slate-900 dark:text-white">{target.label}</strong><span className="mt-1 block text-[10px] leading-4 text-slate-500 dark:text-slate-400">{target.detail}</span><button type="button" className={`${quietButton} mt-4 w-full`} disabled>Configure</button></article>)}</div></section>;
}
...
export function ProductionStudio({ workspaceId, compactLayout, navigationOpen, onCloseNavigation, onNavigate }: ProductionStudioProps) {
  ...
  const workspaceState = primaryWorkspaces.find((workspace) => workspace.id === workspaceId)?.state ?? 'ready';
    ...
    <main className="production-studio flex min-h-0 flex-1 overflow-hidden bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100" id="workspace-main" tabIndex={-1} data-workspace={workspaceId} data-compact={compactLayout ? 'true' : 'false'} data-navigation-position={preferences.navigationPosition} data-navigation-collapsed={collapsed ? 'true' : 'false'}>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex h-11 shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white px-3 dark:border-slate-800 dark:bg-slate-950">
          <div className="flex min-w-0 items-center gap-2"><span className="text-[9px] font-semibold uppercase tracking-[.16em] text-slate-400">{workspaceId === 'editor' ? 'Studio' : 'Workspace'}</span><span className="h-3 w-px bg-slate-200 dark:bg-slate-800" /><strong className="truncate text-[11px] font-semibold text-slate-700 dark:text-slate-200">{workspaceId === 'editor' ? activeDefinition?.label ?? 'Builder' : workspaceLabel}</strong><ImplementationDot state={workspaceId === 'editor' ? activeDefinition?.state ?? 'ready' : workspaceState} /></div>
          <div className="flex items-center gap-1"><button type="button" className={`${quietButton} hidden md:inline-flex`}><Icon name="command" size={13} />Command palette<ImplementationDot state="planned" /></button><button type="button" className={`${quietButton} hidden lg:inline-flex`}><Icon name="users" size={13} />Share<ImplementationDot state="planned" /></button><button type="button" className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-blue-600 px-3 text-[11px] font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30" disabled={workspaceId === 'export'}><Icon name="export" size={13} />Publish<ImplementationDot state="planned" /></button></div>
        </div>

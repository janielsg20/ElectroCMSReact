...
export function GlobalSystemsWorkspace({ initialView = 'themes' }: GlobalSystemsWorkspaceProps) {
  ...
  const { preferences, setDensity, setEditorThemeMode } = useWorkspacePreferences();
    ...
    <section className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--color-ec-app)]" aria-label="Global systems studio">
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-[1480px] flex-1 flex-col p-3 md:p-4">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pt-3">
          ...
          {view === 'blueprints' ? <div><div className="mb-3 flex justify-end"><CapabilityStatus label="Catalog only" detail="Blueprint application is not available until a validated project-level orchestration command can create and connect documents, data models, forms and backend resources atomically." /></div><div className="grid gap-3 lg:grid-cols-3">{blueprintGroups.map((group) => <section key={group.title} className="rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-3 shadow-[var(--ec-shadow-panel)]"><span className="text-[8px] font-bold uppercase tracking-[.12em] text-[var(--color-ec-text-muted)]">{group.title}</span><div className="mt-3 space-y-1">{group.items.map((item) => <article key={item} className="flex min-h-11 items-center justify-between gap-3 rounded-[var(--ec-radius-md)] border border-[var(--color-ec-border)] px-2.5"><span className="flex min-w-0 items-center gap-2"><span className="grid size-7 shrink-0 place-items-center rounded-[var(--ec-radius-sm)] bg-[var(--color-ec-surface-muted)] text-[var(--color-ec-text-muted)]"><Icon name="blueprint" size={12} /></span><strong className="truncate text-[9px] font-semibold text-[var(--color-ec-text)]">{item}</strong></span><span className="shrink-0 rounded-[var(--ec-radius-sm)] bg-[var(--color-ec-surface-subtle)] px-2 py-1 text-[10px] font-semibold text-[var(--color-ec-text-muted)]">Catalog only</span></article>)}</div></section>)}</div></div> : null}
          ...
          {view === 'appearance' ? <div className="grid gap-3 lg:grid-cols-3"><label className="rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-3 text-[9px] font-semibold text-[var(--color-ec-text-muted)] shadow-[var(--ec-shadow-panel)]"><span className="mb-2 block">Editor appearance</span><select className="ec-control h-9 w-full px-2 text-[9px]" aria-label="Global editor theme mode" value={preferences.editorThemeMode} onChange={(event) => setEditorThemeMode(event.target.value as typeof preferences.editorThemeMode)}><option value="auto">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label><label className="rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-3 text-[9px] font-semibold text-[var(--color-ec-text-muted)] shadow-[var(--ec-shadow-panel)]"><span className="mb-2 block">Density</span><select className="ec-control h-9 w-full px-2 text-[9px]" aria-label="Global workspace density" value={preferences.density} onChange={(event) => setDensity(event.target.value as 'compact' | 'comfortable')}><option value="compact">Compact</option><option value="comfortable">Comfortable</option></select></label><section className="rounded-[var(--ec-radius-lg)] border border-[var(--color-ec-border)] bg-[var(--color-ec-surface)] p-3 shadow-[var(--ec-shadow-panel)]" aria-label="Unified editor visual system"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-[var(--ec-radius-md)] bg-[var(--color-ec-accent-soft)] text-[var(--color-ec-accent)]"><Icon name="editor" size={16} /></span><div className="min-w-0"><span className="block text-[8px] font-bold uppercase tracking-[.12em] text-[var(--color-ec-text-muted)]">Visual system</span><strong className="mt-1 block text-[11px] text-[var(--color-ec-text)]">Studio Pro</strong><p className="mt-1 text-[8px] leading-4 text-[var(--color-ec-text-muted)]">One Tailwind-first professional authoring system. Desktop uses persistent builder panels; compact layouts switch to a canvas-first dock and accessible sheets.</p></div></div></section></div> : null}
        </div>
      </div>
    </section>
  );

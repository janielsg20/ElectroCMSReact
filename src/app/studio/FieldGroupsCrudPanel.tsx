...
function descriptorLabel(key: string): string {
...
function FieldConfigEditor({
  ...
  if (descriptors.length === 0) {
    return <p className="text-[10px] leading-4 text-[var(--color-ec-text-muted)]">This field type has no type-specific settings.</p>;
  }
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2">
      {descriptors.map(([key, rawDescriptor]) => {
        ...
        const label = descriptorLabel(key);
          ...
          return (
            <label key={key} className="sm:col-span-2 text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
              ...
              <select
                ...
                onChange={(event) => onChange({ ...field, config: configWithValue(field.config, key, event.target.value) })}
              >
                <option value="">Select Field Group…</option>
                {fieldGroups.filter((group) => group.id !== currentGroupId).map((group) => (
                  <option key={group.id} value={group.id}>{group.label} · {group.id}</option>
                ))}
              </select>
              <small className="mt-1 block font-normal">Only saved reusable groups can be referenced. Cycles and depth &gt; 8 are rejected by core.</small>
            </label>
          );
        }
          ...
          return (
            <label key={key} className="sm:col-span-2 text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
              ...
              >
                ...
                {siblingFields
                  .filter((candidate) => candidate.id !== field.id && !isMf042AdvancedField(candidate))
                  .map((candidate) => <option key={candidate.id} value={candidate.name}>{candidate.label} · {candidate.name}</option>)}
              </select>
              <small className="mt-1 block font-normal">Conditional sources must be non-advanced siblings.</small>
            </label>
          );
        }
        ...
        if (descriptor === 'calculation-expression') {
          return (
            <label key={key} className="sm:col-span-2 text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
              <span className="mb-1 block">{label}</span>
              <input
                className="ec-control h-8 w-full px-2 font-mono text-[10px]"
                aria-label={`Field config ${label}`}
                value={typeof currentValue === 'string' ? currentValue : ''}
        ...
        if (descriptor === 'json?') {
          return (
            <label key={key} className="sm:col-span-2 text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
              <span className="mb-1 block">{label} · JSON</span>
              <textarea
                key={JSON.stringify(currentValue)}
                className="ec-control min-h-16 w-full resize-y px-2.5 py-2 font-mono text-[9px]"
                aria-label={`Field config ${label}`}
                defaultValue={currentValue === undefined ? '' : JSON.stringify(currentValue, null, 2)}
                onBlur={(event) => {
                  const text = event.target.value.trim();
                  if (!text) {
                    onChange({ ...field, config: configWithValue(field.config, key, undefined) });
                    return;
                  }
                  try {
                    onChange({ ...field, config: configWithValue(field.config, key, JSON.parse(text) as JsonValue) });
                  } catch {
                    // Keep last valid config. Canonical validation remains authoritative.
                  }
                }}
              />
            </label>
...
function DefaultValueEditor({ definition, field, onChange }: { definition: FieldTypeDefinition; field: CustomFieldDefinition; onChange(next: CustomFieldDefinition): void }) {
  ...
  return (
    <label className="block text-[10px] font-semibold text-[var(--color-ec-text-muted)]">
      ...
      <textarea key={JSON.stringify(field.defaultValue)} className="ec-control min-h-20 w-full resize-y px-2.5 py-2 font-mono text-[9px]" aria-label="Field default value JSON" defaultValue={JSON.stringify(field.defaultValue, null, 2)} onBlur={(event) => { try { onChange({ ...field, defaultValue: JSON.parse(event.target.value) as JsonValue }); } catch { /* keep last valid value */ } }} />
    </label>
  );
}
...
export function FieldGroupsCrudPanel({ query }: FieldGroupsCrudPanelProps) {
  ...
  const selectDefinition = (definition: FieldGroupDefinition) => {
    setSelectedId(definition.id); setCreating(false); setDraft(structuredClone(definition)); setSelectedFieldIndex(definition.fields.length ? 0 : null); setMessage(null); setDeleteArmed(false);
  };
  ...
  const removeField = (index: number) => {
    ...
    setSelectedFieldIndex((current) => current === null || draft.fields.length <= 1 ? null : Math.min(current, draft.fields.length - 2));
    setMessage(null);
  };
  const moveField = (index: number, direction: -1 | 1) => {
    ...
    const current = fields[index]; const replacement = fields[target];
    if (!current || !replacement) return;
    fields[index] = replacement; fields[target] = current;
    updateDraft('fields', fields); setSelectedFieldIndex(target);
  };
  const changeFieldType = (field: CustomFieldDefinition, type: string): CustomFieldDefinition => {
    ...
    return { ...replacement, name: field.name, description: field.description, required: field.required, conditions: structuredClone(field.conditions), roleVisibility: [...field.roleVisibility], placeholder: definition.features.placeholder === 'supported' ? field.placeholder : null };
  };

  const save = () => {
    const result = creating ? session.createFieldGroup(draft) : selected ? session.updateFieldGroup(selected.id, draft) : null;
    if (!result) return;
    if (!result.ok) { setMessage({ tone: 'error', text: result.message }); return; }
    setCreating(false); setSelectedId(result.value.id); setDraft(structuredClone(result.value)); setMessage({ tone: 'success', text: result.changed ? 'Field group saved.' : 'No changes to save.' });
  };
  const remove = () => {
    if (!selected) return;
    if (!deleteArmed) { setDeleteArmed(true); setMessage(null); return; }
    const result = session.removeFieldGroup(selected.id);
    if (!result.ok) { setDeleteArmed(false); setMessage({ tone: 'error', text: result.message }); return; }
    setDeleteArmed(false); setSelectedId(null); setSelectedFieldIndex(null); setDraft(createDraft()); setMessage({ tone: 'success', text: `Deleted ${selected.label}.` });
  };
          ...
          <div className="space-y-4 p-3">
            ...
            {selectedField && selectedType && selectedFieldIndex !== null ? (
              <section className="space-y-3 rounded-[var(--ec-radius-md)] border border-[var(--color-ec-border)] p-3" aria-label="Selected field editor">
                ...
                <FieldConfigEditor definition={selectedType} field={selectedField} fieldGroups={definitions} currentGroupId={draft.id} siblingFields={draft.fields} onChange={(next) => updateField(selectedFieldIndex, next)} />

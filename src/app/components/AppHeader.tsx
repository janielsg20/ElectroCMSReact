    ...
    <header className="app-header" data-testid="app-header">
      <div className="header-project">
        ...
        <div className="project-identity">
          ...
          <span className="save-state" data-state={session.saveState}><span className="save-dot" aria-hidden="true" />{saveLabels[session.saveState]}</span>
        </div>
      </div>
      ...
      <div className="header-controls" aria-label="Editor controls">
        <label className="compact-field">
          ...
          <select aria-label="Active document" value={session.activeDocumentId} onChange={(event) => session.setActiveDocumentId(event.target.value)}>
        ...
        <label className="compact-field breakpoint-field">
          ...
          <select aria-label="Preview breakpoint" value={session.activeBreakpointId} onChange={(event) => session.setActiveBreakpointId(event.target.value)}>
        ...
        <div className="segmented-control zoom-control" aria-label="Canvas zoom">
          <button type="button" aria-label="Zoom out" disabled={session.zoom <= 50} onClick={() => session.setZoom(session.zoom - 10)}><Icon name="minus" size={14} /></button>
          <output aria-label="Zoom level">{session.zoom}%</output>
          <button type="button" aria-label="Zoom in" disabled={session.zoom >= 200} onClick={() => session.setZoom(session.zoom + 10)}><Icon name="plus" size={14} /></button>
        </div>

        <div className="segmented-control history-control" aria-label="Document history">
          <button type="button" aria-label="Undo" disabled={!session.canUndo} title={session.canUndo ? 'Undo last document command' : 'Nothing to undo'} onClick={session.undo}><Icon name="undo" size={14} /></button>
          <button type="button" aria-label="Redo" disabled={!session.canRedo} title={session.canRedo ? 'Redo last document command' : 'Nothing to redo'} onClick={session.redo}><Icon name="redo" size={14} /></button>
        </div>
        ...
        <label className="compact-field theme-field">
          ...
          <select aria-label="Editor theme mode" value={preferences.editorThemeMode} onChange={(event) => setEditorThemeMode(event.target.value as 'light' | 'dark' | 'auto')}>

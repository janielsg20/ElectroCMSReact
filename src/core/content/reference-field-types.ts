...
function rejectUnknown(config: JsonObject, allowed: readonly string[]): FieldTypeValidationResult | null {
  ...
  return key ? invalidFieldTypeValue('UNKNOWN_REFERENCE_CONFIG', key, `Unsupported reference field config key: ${key}.`) : null;
}
...
function validateRelationConfig(config: JsonObject): FieldTypeValidationResult {
  ...
  if (typeof config.relationId !== 'string' || !config.relationId.trim()) issues.push({ code: 'INVALID_RELATION_ID', path: 'relationId', message: 'relationId must be a non-empty string.' });
  if (!RELATION_FIELD_SIDES.includes(config.side as RelationFieldSide)) issues.push({ code: 'INVALID_RELATION_SIDE', path: 'side', message: 'side must be source or target.' });
...
function validateReferenceList(value: JsonValue): FieldTypeValidationResult {
  if (!Array.isArray(value) || value.some((item) => typeof item !== 'string' || !item.trim())) return invalidFieldTypeValue('INVALID_REFERENCE_LIST', '$', 'Reference value must be an array of non-empty string ids.');
...
function definition(input: {
  ...
  return {
    type: input.type,
    version: 2,
    metadata: { label: input.label, category: 'reference', icon: input.icon, description: input.description, keywords: ['reference', 'relation', 'mf-043'] },
...
export function createMf043ReferenceFieldTypeDefinitions(): FieldTypeDefinition[] {
  return [
    definition({ type: 'core/relation', label: 'Relation', icon: 'git-branch', description: 'Reference canonical Records through a configured Content Relation.', valueShape: 'array', configSchema: { relationId: 'relation-id', side: 'relation-side' }, defaultConfig: { relationId: '', side: 'source' }, validateConfig: validateRelationConfig, validateValue: validateReferenceList, createDefaultValue: () => [] }),
    definition({ type: 'core/user', label: 'User', icon: 'user-round', description: 'Reference one user from CanonicalProject.users.', valueShape: 'string', configSchema: {}, defaultConfig: {}, validateConfig: validateEmptyConfig, validateValue: validateUserReference, createDefaultValue: () => null }),
    definition({ type: 'core/taxonomy', label: 'Taxonomy', icon: 'tags', description: 'Store unique term ids scoped to one configured Taxonomy definition.', valueShape: 'array', configSchema: { taxonomyId: 'taxonomy-id' }, defaultConfig: { taxonomyId: '' }, validateConfig: validateTaxonomyConfig, validateValue: validateReferenceList, createDefaultValue: () => [] }),
  ];
}
...
export function isMf043ReferenceField(field: Pick<CustomFieldDefinition, 'type' | 'typeVersion'>): boolean {
...
export function validateReferenceFieldContext(project: CanonicalProject, ownerContentTypeId: string, field: CustomFieldDefinition, value: JsonValue): string[] {
  if (!isMf043ReferenceField(field)) return [];
  if (field.type === 'core/user') {
    if (value === null) return [];
    return typeof value === 'string' && value in project.users ? [] : [`User ${String(value)} does not exist.`];
  }
  if (field.type === 'core/taxonomy') {
    ...
    if (!taxonomy.contentTypeIds.includes(ownerContentTypeId)) return [`Taxonomy ${taxonomy.id} is not assigned to Content Type ${ownerContentTypeId}.`];
    return [];
  }
  ...
  if (ownerContentTypeId !== ownerExpected) return [`Relation ${relation.id} expects a ${ownerExpected} record on its ${side} side.`];
  ...
  for (const recordId of value) {
    ...
    if (!record) { errors.push(`Referenced record ${recordId} does not exist.`); continue; }
    if (record.contentTypeId !== referencedExpected) errors.push(`Referenced record ${recordId} must belong to Content Type ${referencedExpected}.`);
  }
  return errors;
}

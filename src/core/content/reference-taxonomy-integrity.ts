...
export function removeTaxonomyWithReferenceIntegrity(project: CanonicalProject, id: string): TaxonomyMutationResult {
  ...
  if (referencingGroup) {
    return { ok: false, error: { code: 'PROJECT_INVALID', message: `Taxonomy ${id} is referenced by Field Group ${referencingGroup[0]} and cannot be deleted until that field is removed or migrated.` } };
  }

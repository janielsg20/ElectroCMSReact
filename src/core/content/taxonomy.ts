...
export function validateTaxonomyDefinition(input: unknown): TaxonomyValidationResult {
  ...
  if (input.version !== TAXONOMY_DEFINITION_VERSION) {
    issues.push({ code: 'INVALID_VERSION', path: 'version', message: `Taxonomy version must be ${TAXONOMY_DEFINITION_VERSION}.` });
  }
  if (!TAXONOMY_ID_PATTERN.test(id) || id.length > 64) {
    issues.push({ code: 'INVALID_ID', path: 'id', message: 'Taxonomy id must be kebab-case, begin with a letter and be at most 64 characters.' });
  }
  if (!label || label.length > MAX_LABEL_LENGTH) {
    issues.push({ code: 'INVALID_LABEL', path: 'label', message: `Label is required and must be at most ${MAX_LABEL_LENGTH} characters.` });
  }
  if (!singularLabel || singularLabel.length > MAX_LABEL_LENGTH) {
    issues.push({ code: 'INVALID_SINGULAR_LABEL', path: 'singularLabel', message: `Singular label is required and must be at most ${MAX_LABEL_LENGTH} characters.` });
  }
  if (!TAXONOMY_SLUG_PATTERN.test(slug) || slug.length > 80) {
    issues.push({ code: 'INVALID_SLUG', path: 'slug', message: 'Slug must be lowercase kebab-case and at most 80 characters.' });
  }
  if (description.length > MAX_DESCRIPTION_LENGTH) {
    issues.push({ code: 'INVALID_DESCRIPTION', path: 'description', message: `Description must be at most ${MAX_DESCRIPTION_LENGTH} characters.` });
  }
  if (typeof input.hierarchical !== 'boolean') {
    issues.push({ code: 'INVALID_HIERARCHICAL', path: 'hierarchical', message: 'Hierarchical must be boolean.' });
  }
  if (!contentTypeIds || contentTypeIds.length === 0 || hasDuplicates(contentTypeIds)) {
    issues.push({ code: 'INVALID_CONTENT_TYPES', path: 'contentTypeIds', message: 'Taxonomy must target one or more unique content type ids.' });
  }
  if (!fieldGroupIds || hasDuplicates(fieldGroupIds)) {
    issues.push({ code: 'INVALID_FIELD_GROUPS', path: 'fieldGroupIds', message: 'Field group ids must be a unique string list.' });
  }
  if (input.archiveTemplateId !== null && !archiveTemplateId) {
    issues.push({ code: 'INVALID_ARCHIVE_TEMPLATE', path: 'archiveTemplateId', message: 'Archive template id must be null or a non-empty string.' });
  }
...
function slugBelongsToAnotherTaxonomy(project: CanonicalProject, slug: string, excludedId?: string): boolean {
  return listTaxonomyDefinitions(project).some((definition) => definition.id !== excludedId && definition.slug === slug);
}

function validateReferences(project: CanonicalProject, definition: TaxonomyDefinition): TaxonomyMutationError | null {
  ...
  if (unknownContentType) {
    return { code: 'UNKNOWN_CONTENT_TYPE', message: `Taxonomy references unknown content type ${unknownContentType}.` };
  }

  const unknownFieldGroup = definition.fieldGroupIds.find((id) => !(id in project.fieldGroups));
  if (unknownFieldGroup) {
    return { code: 'UNKNOWN_FIELD_GROUP', message: `Taxonomy references unknown field group ${unknownFieldGroup}.` };
  }
  ...
  if (definition.archiveTemplateId !== null) {
    ...
    if (!document || document.kind !== 'archive') {
      return { code: 'UNKNOWN_ARCHIVE_TEMPLATE', message: `Taxonomy archive template ${definition.archiveTemplateId} is not an archive document.` };
    }
  }
...
export function createTaxonomy(project: CanonicalProject, input: unknown): TaxonomyMutationResult {
  ...
  return withValidatedProject(project, { ...project.taxonomies, [definition.id]: serializeTaxonomyDefinition(definition) }, definition);
}

export function updateTaxonomy(project: CanonicalProject, id: string, input: unknown): TaxonomyMutationResult {
  ...
  return withValidatedProject(project, { ...project.taxonomies, [id]: serializeTaxonomyDefinition(definition) }, definition);
}
...
export function removeTaxonomy(project: CanonicalProject, id: string): TaxonomyMutationResult {
  ...
  if (!current) return { ok: false, error: { code: 'NOT_FOUND', message: `Taxonomy ${id} was not found.` } };
  const validation = validateTaxonomyDefinition(current);
  if (!validation.ok) {
    return { ok: false, error: { code: 'INVALID_DEFINITION', message: 'Existing taxonomy definition is invalid.', issues: validation.issues } };
  }

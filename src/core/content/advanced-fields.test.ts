...
function project() {
  const base = createCanonicalProject({
    id: 'advanced_fields_test',
    name: 'Advanced Fields Test',
...
function field(
  ...
  return {
    ...createDefaultCustomFieldDefinition(registry, type, id, label),
    ...patch,
  };
}
...
function addGroup(
  ...
  const definition: FieldGroupDefinition = {
    ...createDefaultFieldGroupDefinition(id, label),
    fields,
  };
...
function advancedProject() {
  ...
  current = addGroup(current, 'order-data', 'Order data', [
    ...
    field('core/calculated', 'subtotal', 'Subtotal', {
      name: 'subtotal',
      config: { expression: 'quantity * unit_price' },
      defaultValue: 0,
    }),
    field('core/switch', 'show-extra', 'Show extra', { name: 'show_extra', defaultValue: false }),
    field('core/group', 'shipping', 'Shipping', {
      name: 'shipping',
      config: { fieldGroupId: 'address-fields' },
      defaultValue: {},
    }),
    field('core/repeater', 'items', 'Items', {
      name: 'items',
      config: { fieldGroupId: 'line-item-fields', minItems: 1, maxItems: 5 },
      defaultValue: [],
    }),
    field('core/conditional', 'extra-address', 'Extra address', {
      name: 'extra_address',
      config: {
        fieldGroupId: 'address-fields',
        sourceField: 'show_extra',
        operator: 'truthy',
      },
      defaultValue: null,
    }),
...
describe('MF-042 advanced fields', () => {
  ...
  it('creates reusable group/repeater/conditional schemas and blocks reference cycles', () => {
    ...
    const address = {
      ...
      fields: [
        field('core/text', 'city', 'City', { name: 'city', required: true }),
        field('core/group', 'recursive-order', 'Recursive order', {
          name: 'recursive_order',
          config: { fieldGroupId: 'order-data' },
          defaultValue: {},
        }),
      ],
    };
  ...
  it('normalizes calculated/conditional values and validates nested group and repeater rows', () => {
    ...
    const draft = {
      ...
      fieldValues: {
        'order-data': {
          ...
          subtotal: 999,
          show_extra: false,
          shipping: { city: 'Houston', state: 'TX' },
          items: [{ name: 'Main item', amount: 2 }],
          extra_address: { city: 'Should be cleared' },
        },
      },
    };
    ...
    expect(validation.value.fieldValues['order-data']).toMatchObject({
      subtotal: 25,
      extra_address: null,
    });

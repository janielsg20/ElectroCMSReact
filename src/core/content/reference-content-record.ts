...
function referenceIssuesInGroup(
  ...
  for (const field of group.fields) {
    ...
    if (isMf043ReferenceField(field)) {
      for (const message of validateReferenceFieldContext(project, ownerContentTypeId, field, value)) {
        issues.push({ code: 'INVALID_FIELD_VALUE', path: fieldPath, message });
      }
      continue;
    }
    ...
    if (field.type === 'core/repeater' && Array.isArray(value)) {
      value.forEach((row, index) => {
        if (isJsonObject(row)) {
          issues.push(...referenceIssuesInGroup(project, ownerContentTypeId, nestedGroup, row, groups, `${fieldPath}.${index}`));
        }
      });
    }
  }
...
function referenceIssues(
  ...
  for (const groupId of record.fieldGroupIds) {
    ...
    issues.push(...referenceIssuesInGroup(
      project,
...
function referencedRecordIdsInGroup(
  group: FieldGroupDefinition,
  values: JsonObject,
  groups: ReadonlyMap<string, FieldGroupDefinition>,
  output: Set<string>,
  ...
  for (const field of group.fields) {
    ...
    if (!nested) continue;
    if ((field.type === 'core/group' || field.type === 'core/conditional') && isJsonObject(value)) {
      referencedRecordIdsInGroup(nested, value, groups, output);
    } else if (field.type === 'core/repeater' && Array.isArray(value)) {
      value.forEach((row) => {
        if (isJsonObject(row)) referencedRecordIdsInGroup(nested, row, groups, output);
      });
    }
  }
}
...
export function validateReferenceContentRecordDefinition(
  input: unknown,
  project: CanonicalProject,
...
export function createReferenceContentRecord(
  project: CanonicalProject,
  input: unknown,
  registry: FieldTypeRegistry = createContentFieldTypeRegistry(),
): ContentRecordMutationResult {
...
export function updateReferenceContentRecord(
  project: CanonicalProject,
  id: string,
  input: unknown,
  registry: FieldTypeRegistry = createContentFieldTypeRegistry(),
...
export function removeReferenceContentRecord(
  project: CanonicalProject,
  id: string,
  registry: FieldTypeRegistry = createContentFieldTypeRegistry(),
): ContentRecordMutationResult {
  const referencing = listAdvancedContentRecords(project, {}, registry).find(
    (record) => record.id !== id && recordReferences(project, record, registry).has(id),
  );
  if (referencing) {
    return {
      ok: false,
...
export function listReferenceContentRecords(
  project: CanonicalProject,
  filter: ContentRecordListOptions = {},
  registry: FieldTypeRegistry = createContentFieldTypeRegistry(),
): ContentRecordDefinition[] {
  return listAdvancedContentRecords(project, filter, registry).filter(
    (record) => validateReferenceContentRecordDefinition(record, project, registry).ok,
  );
}

...
function recordUsesFieldGroup(raw: unknown, fieldGroupId: string, groups: ReadonlyMap<string, FieldGroupDefinition>): boolean {
  if (!isJsonObject(raw) || !Array.isArray(raw.fieldGroupIds)) return false;
  return raw.fieldGroupIds.some((candidate) => typeof candidate === 'string' && groupDependsOn(candidate, fieldGroupId, groups));
}
...
export function updateFieldGroupWithRecordIntegrity(
  ...
  const groups = new Map(listAdvancedFieldGroupDefinitions(updated.project, registry).map((group) => [group.id, group]));

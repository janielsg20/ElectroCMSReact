...
function normalizeFieldValues(
  ...
  for (const groupId of Object.keys(input)) {
    if (!allowedGroupIds.has(groupId)) {
      addIssue(issues, 'UNKNOWN_FIELD_VALUE', `fieldValues.${groupId}`, `Values were provided for unselected field group ${groupId}.`);
    }
  }
  ...
  for (const groupId of fieldGroupIds) {
    ...
    if (!group) {
      addIssue(issues, 'UNKNOWN_FIELD_GROUP', 'fieldGroupIds', `Field group ${groupId} does not exist or is invalid.`);
      continue;
    }

    const rawGroupValues = input[groupId];
    if (rawGroupValues !== undefined && !isJsonObject(rawGroupValues)) {
      addIssue(issues, 'INVALID_FIELD_VALUES', `fieldValues.${groupId}`, `Values for ${group.label} must be a portable JSON object.`);
      continue;
    }
    ...
    for (const key of Object.keys(source)) {
      if (!allowedNames.has(key)) {
        addIssue(issues, 'UNKNOWN_FIELD_VALUE', `fieldValues.${groupId}.${key}`, `Field ${key} is not defined by ${group.label}.`);
      }
    }
    ...
    for (const field of group.fields) {
      ...
      if (!isJsonValue(candidate)) {
        addIssue(issues, 'INVALID_FIELD_VALUE', `fieldValues.${groupId}.${field.name}`, `${field.label} must be portable JSON.`);
        continue;
      }
      if (field.required && isRequiredValueMissing(candidate)) {
        addIssue(issues, 'REQUIRED_FIELD_MISSING', `fieldValues.${groupId}.${field.name}`, `${field.label} is required.`);
      }
      try {
        const validation = registry.validateValue(field.type, candidate, field.config, field.typeVersion);
...
export function validateContentRecordDefinition(
  ...
  if (!isJsonObject(input)) {
    return { ok: false, issues: [{ code: 'INVALID_OBJECT', path: '$', message: 'Record must be a portable JSON object.' }] };
  }
  ...
  if (input.version !== CONTENT_RECORD_VERSION) addIssue(issues, 'INVALID_VERSION', 'version', `Record version must be ${CONTENT_RECORD_VERSION}.`);
  if (!RECORD_ID_PATTERN.test(id) || id.length > 80) addIssue(issues, 'INVALID_ID', 'id', 'Record id must be kebab-case and at most 80 characters.');
  if (!contentType) addIssue(issues, 'INVALID_CONTENT_TYPE', 'contentTypeId', `Content type ${contentTypeId || '(empty)'} does not exist.`);
  if (!CONTENT_RECORD_STATUSES.includes(input.status as ContentRecordStatus)) addIssue(issues, 'INVALID_STATUS', 'status', 'Record status must be draft, published or archived.');
  if (typeof input.title !== 'string' || title.length > MAX_TITLE_LENGTH || (contentType?.supports.title && !title)) {
    addIssue(issues, 'INVALID_TITLE', 'title', `Title must be a string of at most ${MAX_TITLE_LENGTH} characters${contentType?.supports.title ? ' and cannot be empty' : ''}.`);
  }
  if (!SLUG_PATTERN.test(slug) || slug.length > MAX_SLUG_LENGTH) addIssue(issues, 'INVALID_SLUG', 'slug', `Slug must be lowercase kebab-case and at most ${MAX_SLUG_LENGTH} characters.`);
  if (typeof input.excerpt !== 'string' || excerpt.length > MAX_EXCERPT_LENGTH) addIssue(issues, 'INVALID_EXCERPT', 'excerpt', `Excerpt must be at most ${MAX_EXCERPT_LENGTH} characters.`);
  if (typeof input.content !== 'string' || content.length > MAX_CONTENT_LENGTH) addIssue(issues, 'INVALID_CONTENT', 'content', `Content must be at most ${MAX_CONTENT_LENGTH} characters.`);
  if (!fieldGroupIds) addIssue(issues, 'INVALID_FIELD_GROUPS', 'fieldGroupIds', `Field group ids must be a unique string array with at most ${MAX_FIELD_GROUPS} entries.`);
  if (!isIsoTimestamp(input.createdAt)) addIssue(issues, 'INVALID_CREATED_AT', 'createdAt', 'createdAt must be a canonical ISO timestamp.');
  if (!isIsoTimestamp(input.updatedAt)) addIssue(issues, 'INVALID_UPDATED_AT', 'updatedAt', 'updatedAt must be a canonical ISO timestamp.');
  ...
  if (
    issues.length > 0 || !contentType || !fieldGroupIds || !normalizedFieldValues ||
    !isIsoTimestamp(input.createdAt) || !isIsoTimestamp(input.updatedAt) ||
...
export function serializeContentRecordDefinition(record: ContentRecordDefinition): JsonObject {
  ...
  for (const [groupId, values] of Object.entries(record.fieldValues)) fieldValues[groupId] = structuredClone(values);
  return {
...
function slugBelongsToAnotherRecord(project: CanonicalProject, contentTypeId: string, slug: string, excludedId?: string): boolean {
...
export function createContentRecord(
  ...
  if (!validation.ok) return { ok: false, error: { code: 'INVALID_DEFINITION', message: validation.issues.map((issue) => issue.message).join(' '), issues: validation.issues } };
  const record = validation.value;
  if (record.id in project.records) return { ok: false, error: { code: 'DUPLICATE_ID', message: `Record ${record.id} already exists.` } };
  if (slugBelongsToAnotherRecord(project, record.contentTypeId, record.slug)) return { ok: false, error: { code: 'DUPLICATE_SLUG', message: `Slug ${record.slug} is already used by this content type.` } };
  return withValidatedProject(project, { ...project.records, [record.id]: serializeContentRecordDefinition(record) }, record);
}
...
export function updateContentRecord(
  ...
  if (!existingRaw) return { ok: false, error: { code: 'NOT_FOUND', message: `Record ${id} was not found.` } };
  const existingValidation = validateContentRecordDefinition(existingRaw, project, registry);
  if (!existingValidation.ok) return { ok: false, error: { code: 'INVALID_DEFINITION', message: 'Existing record is invalid.', issues: existingValidation.issues } };
  const validation = validateContentRecordDefinition(input, project, registry);
  if (!validation.ok) return { ok: false, error: { code: 'INVALID_DEFINITION', message: validation.issues.map((issue) => issue.message).join(' '), issues: validation.issues } };
  const candidate = validation.value;
  if (candidate.id !== id) return { ok: false, error: { code: 'ID_MISMATCH', message: 'Record id is immutable after creation.' } };
  if (candidate.createdAt !== existingValidation.value.createdAt) return { ok: false, error: { code: 'CREATED_AT_MISMATCH', message: 'Record createdAt is immutable.' } };
  if (slugBelongsToAnotherRecord(project, candidate.contentTypeId, candidate.slug, id)) return { ok: false, error: { code: 'DUPLICATE_SLUG', message: `Slug ${candidate.slug} is already used by this content type.` } };
  const updated: ContentRecordDefinition = { ...candidate, updatedAt: new Date().toISOString() };
  return withValidatedProject(project, { ...project.records, [id]: serializeContentRecordDefinition(updated) }, updated);
}
...
export function removeContentRecord(
  ...
  if (!validation.ok) return { ok: false, error: { code: 'INVALID_DEFINITION', message: 'Existing record is invalid.', issues: validation.issues } };

...
describe('taxonomy model', () => {
  ...
  it('rejects duplicate ids and slugs', () => {
    ...
    const duplicateId = createTaxonomy(
      first.project,
      createDefaultTaxonomyDefinition('categories', 'Other', ['products']),
    );
  ...
  it('rejects unknown content type, field group and archive template references', () => {
    const project = projectWithProducts();
    const unknownType = createTaxonomy(
      project,
      createDefaultTaxonomyDefinition('categories', 'Categories', ['missing']),
    );

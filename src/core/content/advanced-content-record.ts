...
function normalizeAdvancedRecordInput(project: CanonicalProject, input: unknown, registry: FieldTypeRegistry): unknown {
  if (!isJsonObject(input) || !Array.isArray(input.fieldGroupIds) || !isJsonObject(input.fieldValues)) return input;
  ...
  for (const groupId of input.fieldGroupIds) {
    ...
    for (const field of group.fields) {
      ...
      values[field.name] = isMf042AdvancedField(field)
        ? createAdvancedFieldDefaultValue(field, { registry, resolveGroup: (id) => groups.get(id) ?? null, currentValues: values })
    ...
    for (const field of group.fields) {
      ...
      if (isJsonValue(value)) values[field.name] = normalizeAdvancedFieldValue(field, value, { registry, resolveGroup: (id) => groups.get(id) ?? null, currentValues: values });
    }
    for (const field of group.fields) {
...
function advancedRecordIssues(project: CanonicalProject, record: ContentRecordDefinition, registry: FieldTypeRegistry): ContentRecordValidationIssue[] {
  ...
  for (const groupId of record.fieldGroupIds) {
    ...
    for (const field of group.fields) {
      ...
      for (const item of validateAdvancedFieldValue(field, value, { registry, resolveGroup: (id) => groups.get(id) ?? null, currentValues: values })) {
        issues.push({ code: 'INVALID_FIELD_VALUE', path: `fieldValues.${group.id}.${field.name}${item.path === '$' ? '' : `.${item.path}`}`, message: item.message });
      }
    }
  }
  return issues;
...
export function validateAdvancedContentRecordDefinition(input: unknown, project: CanonicalProject, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): ContentRecordValidationResult {
...
function mutationError(validation: { ok: false; issues: readonly ContentRecordValidationIssue[] }): ContentRecordMutationResult {
  return { ok: false, error: { code: 'INVALID_DEFINITION', message: validation.issues.map((item) => item.message).join(' '), issues: validation.issues } };
}

export function createAdvancedContentRecord(project: CanonicalProject, input: unknown, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): ContentRecordMutationResult {
  const normalized = normalizeAdvancedRecordInput(project, input, registry);
...
export function updateAdvancedContentRecord(project: CanonicalProject, id: string, input: unknown, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): ContentRecordMutationResult {
  const normalized = normalizeAdvancedRecordInput(project, input, registry);
  const validation = validateAdvancedContentRecordDefinition(normalized, project, registry);
  if (!validation.ok) return mutationError(validation);
...
export function removeAdvancedContentRecord(project: CanonicalProject, id: string, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): ContentRecordMutationResult {
  return removeBaseRecord(project, id, registry);
}

export function listAdvancedContentRecords(project: CanonicalProject, filter: ContentRecordListOptions = {}, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): ContentRecordDefinition[] {
  return listBaseRecords(project, filter, registry).filter((record) => validateAdvancedContentRecordDefinition(record, project, registry).ok);
}

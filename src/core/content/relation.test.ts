...
describe('MF-043 relation model', () => {
  it('creates lists and updates a relation between canonical Content Types', () => {
    ...
    const updated = updateRelation(created.project, definition.id, { ...definition, label: 'Catalog brand', bidirectional: false });
  ...
  it('rejects missing endpoints and immutable id changes', () => {
    ...
    expect(created.ok).toBe(true);
    if (!created.ok) return;
    const renamed = updateRelation(created.project, 'product-brand', { ...created.value, id: 'renamed-relation' });

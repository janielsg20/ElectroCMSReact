...
export function createRelationWithIntegrity(
...
export function updateRelationWithIntegrity(
  project: CanonicalProject,
  id: string,
  input: unknown,
): RelationMutationResult {
    ...
    if (!validation.ok) {
      ...
      return {
        ...
        error: {
          ...
          message: first
            ? `Relation ${id} cannot be updated because Field Group ${groupId} would become invalid: ${first.message}`
            : `Relation ${id} cannot be updated because Field Group ${groupId} would become invalid.`,
        },
      };
    }
  }
    ...
    if (!validation.ok) {
      ...
      return {
        ...
        error: {
          code: 'RELATION_IN_USE',
          message: first
            ? `Relation ${id} cannot be updated because record ${recordId} would become invalid: ${first.message}`
            : `Relation ${id} cannot be updated because record ${recordId} would become invalid.`,
        },
      };
    }
  }
...
export function removeRelationWithIntegrity(
  project: CanonicalProject,
  id: string,
): RelationMutationResult {

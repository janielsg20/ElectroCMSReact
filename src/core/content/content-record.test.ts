...
function createRecordFixture() {
  ...
  const groupResult = createFieldGroup(project, { ...details, fields: [sku, price, featured] }, registry);
  ...
  const record: ContentRecordDefinition = {
    ...
    fieldValues: {
      'product-details': { sku: 'SKU-001', price: 25, featured: false },
    },
  };
...
describe('content record engine', () => {
  it('validates a record against its CPT and selected field group schemas', () => {
    ...
    expect(validateContentRecordDefinition(record, project, registry)).toEqual({ ok: true, value: record });
  });
  ...
  it('applies stored field defaults but still enforces required values', () => {
    ...
    expect(normalized.value.fieldValues['product-details']).toMatchObject({ sku: 'SKU-002', price: 0, featured: false });
  });
  ...
  it('rejects unknown groups, unselected group payloads and unknown field names', () => {
    ...
    if (!groupResult.ok) expect(groupResult.issues.some((issue) => issue.code === 'UNKNOWN_FIELD_GROUP')).toBe(true);
    ...
    if (!extraResult.ok) expect(extraResult.issues.some((issue) => issue.code === 'UNKNOWN_FIELD_VALUE')).toBe(true);
    ...
    if (!fieldResult.ok) expect(fieldResult.issues.some((issue) => issue.path.endsWith('.unknown'))).toBe(true);
  });
  ...
  it('creates updates lists searches filters and removes records', () => {
    ...
    const updated = updateContentRecord(created.project, record.id, { ...record, status: 'published', title: 'Product Alpha Updated' }, registry);
  ...
  it('rejects duplicate ids and duplicate slugs inside the same CPT but permits the same slug in another CPT', () => {
    ...
    const duplicateSlug = createContentRecord(created.project, { ...record, id: 'product-beta', title: 'Product Beta' }, registry);
    ...
    expect(createContentRecord(created.project, article, registry).ok).toBe(true);
  });
  ...
  it('keeps id and createdAt immutable during updates', () => {
    ...
    const renamed = updateContentRecord(created.project, record.id, { ...record, id: 'renamed-record' }, registry);
    expect(renamed.ok).toBe(false);
    if (!renamed.ok) expect(renamed.error.code).toBe('ID_MISMATCH');

    const changedCreatedAt = updateContentRecord(created.project, record.id, { ...record, createdAt: '2026-08-08T07:41:00.000Z' }, registry);

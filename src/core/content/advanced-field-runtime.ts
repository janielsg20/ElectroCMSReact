...
export function advancedFieldGroupReference(field: CustomFieldDefinition): string | null {
  ...
  if (field.type !== 'core/group' && field.type !== 'core/repeater' && field.type !== 'core/conditional') return null;
...
export function validateAdvancedFieldConfig(field: CustomFieldDefinition): AdvancedFieldRuntimeIssue[] {
  ...
  if (field.type === 'core/group') {
    if (!advancedFieldGroupReference(field)) issues.push(issue('INVALID_GROUP_REFERENCE', 'config.fieldGroupId', 'Group requires a referenced Field Group id.'));
    return issues;
  }

  if (field.type === 'core/repeater') {
    if (!advancedFieldGroupReference(field)) issues.push(issue('INVALID_GROUP_REFERENCE', 'config.fieldGroupId', 'Repeater requires a referenced Field Group id.'));
    const minItems = field.config.minItems;
    const maxItems = field.config.maxItems;
    if (minItems !== undefined && (!Number.isInteger(minItems) || Number(minItems) < 0)) issues.push(issue('INVALID_MIN_ITEMS', 'config.minItems', 'minItems must be a non-negative integer.'));
    if (maxItems !== undefined && (!Number.isInteger(maxItems) || Number(maxItems) < 1)) issues.push(issue('INVALID_MAX_ITEMS', 'config.maxItems', 'maxItems must be a positive integer.'));
    if (typeof maxItems === 'number' && maxItems > MAX_REPEATER_ITEMS) issues.push(issue('REPEATER_LIMIT_EXCEEDED', 'config.maxItems', `maxItems cannot exceed ${MAX_REPEATER_ITEMS}.`));
    if (typeof minItems === 'number' && minItems > MAX_REPEATER_ITEMS) issues.push(issue('REPEATER_LIMIT_EXCEEDED', 'config.minItems', `minItems cannot exceed ${MAX_REPEATER_ITEMS}.`));
    if (typeof minItems === 'number' && typeof maxItems === 'number' && minItems > maxItems) issues.push(issue('INVALID_ITEM_RANGE', 'config', 'minItems cannot exceed maxItems.'));
    return issues;
  }
  ...
  if (!advancedFieldGroupReference(field)) issues.push(issue('INVALID_GROUP_REFERENCE', 'config.fieldGroupId', 'Conditional requires a referenced Field Group id.'));
  ...
  if ((operator === 'equals' || operator === 'notEquals') && !hasCompareValue(field)) issues.push(issue('MISSING_COMPARE_VALUE', 'config.compareValue', `${operator} requires compareValue.`));
  if ((operator === 'greaterThan' || operator === 'lessThan') && (typeof field.config.compareValue !== 'number' || !Number.isFinite(field.config.compareValue))) {
    issues.push(issue('INVALID_COMPARE_VALUE', 'config.compareValue', `${operator} requires a finite numeric compareValue.`));
  }
  return issues;
}

export function createAdvancedFieldDefaultValue(field: CustomFieldDefinition, context: AdvancedFieldRuntimeContext): JsonValue {
...
export function normalizeAdvancedFieldValue(field: CustomFieldDefinition, value: JsonValue, context: AdvancedFieldRuntimeContext): JsonValue {
  ...
  if (field.type === 'core/conditional') {
    ...
    return normalizeGroupObject(group, objectValue, { ...context, currentValues: objectValue, depth: depth + 1 });
  }
  ...
  if (field.type === 'core/repeater') {
    ...
    const group = groupId ? context.resolveGroup(groupId) : null;
    if (!group) return structuredClone(value);
    return value.map((item) => isJsonObject(item)
      ? normalizeGroupObject(group, item, { ...context, currentValues: item, depth: depth + 1 })
...
export function validateAdvancedFieldValue(field: CustomFieldDefinition, value: JsonValue, context: AdvancedFieldRuntimeContext): AdvancedFieldRuntimeIssue[] {
  if (!isMf042AdvancedField(field)) return [];
  const depth = context.depth ?? 0;
  if (depth > MAX_ADVANCED_FIELD_DEPTH) return [issue('ADVANCED_FIELD_DEPTH', '$', `Advanced field nesting exceeds ${MAX_ADVANCED_FIELD_DEPTH} levels.`)];

  if (field.type === 'core/calculated') {
    const result = evaluateCalculatedField(field, context.currentValues ?? {});
    if (!result.ok) return [issue('CALCULATION_FAILED', '$', result.message)];
    return value === result.value ? [] : [issue('STALE_CALCULATED_VALUE', '$', 'Calculated value does not match the current expression inputs.')];
  }
...
function validateNestedGroupValue(field: CustomFieldDefinition, value: JsonValue, context: AdvancedFieldRuntimeContext): AdvancedFieldRuntimeIssue[] {
...
export function normalizeGroupObject(group: FieldGroupDefinition, value: JsonObject, context: AdvancedFieldRuntimeContext): JsonObject {
  const depth = context.depth ?? 0;
  if (depth > MAX_ADVANCED_FIELD_DEPTH) return structuredClone(value);
  ...
  for (const field of group.fields) {
    ...
    values[field.name] = normalizeAdvancedFieldValue(field, candidate, { ...context, currentValues: values, depth });
  }
  for (const field of group.fields) {
...
export function createGroupDefaultValue(group: FieldGroupDefinition, context: AdvancedFieldRuntimeContext): JsonObject {
  return normalizeGroupObject(group, {}, context);
}

export function validateGroupObject(group: FieldGroupDefinition, value: JsonObject, context: AdvancedFieldRuntimeContext): AdvancedFieldRuntimeIssue[] {
  const issues: AdvancedFieldRuntimeIssue[] = [];
  const known = new Set(group.fields.map((field) => field.name));
  for (const key of Object.keys(value)) if (!known.has(key)) issues.push(issue('UNKNOWN_NESTED_FIELD', key, `Field ${key} is not defined by ${group.label}.`));
  const normalizedContext = normalizeGroupObject(group, value, context);

  for (const field of group.fields) {
    const candidate = normalizedContext[field.name];
    if (candidate === undefined) continue;
    if (field.required && requiredValueMissing(candidate)) issues.push(issue('REQUIRED_NESTED_FIELD', field.name, `${field.label} is required.`));
...
function tokenizeCalculation(expression: string): CalcToken[] | null {
  ...
  while (index < expression.length) {
    ...
    if ('+-*/'.includes(char)) { tokens.push({ kind: 'operator', value: char as '+' | '-' | '*' | '/' }); index += 1; continue; }

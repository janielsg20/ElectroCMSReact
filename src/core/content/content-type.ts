...
export function validateContentTypeDefinition(input: unknown): ContentTypeValidationResult {
  ...
  if (input.version !== CONTENT_TYPE_DEFINITION_VERSION) {
    issues.push({ code: 'INVALID_VERSION', path: 'version', message: `Content type version must be ${CONTENT_TYPE_DEFINITION_VERSION}.` });
  }
  if (!CONTENT_TYPE_ID_PATTERN.test(id) || id.length > 64) {
    issues.push({ code: 'INVALID_ID', path: 'id', message: 'Content type id must be kebab-case, begin with a letter and be at most 64 characters.' });
  }
  ...
  if (!supports) {
    issues.push({ code: 'INVALID_SUPPORTS', path: 'supports', message: 'Supports must define boolean title, editor, excerpt and featuredImage flags.' });
  }
...
function slugBelongsToAnotherContentType(project: CanonicalProject, slug: string, excludedId?: string): boolean {
  return listContentTypeDefinitions(project).some((definition) => definition.id !== excludedId && definition.slug === slug);
}
...
export function createContentType(project: CanonicalProject, input: unknown): ContentTypeMutationResult {
  ...
  return withValidatedProject(project, { ...project.contentTypes, [definition.id]: serializeContentTypeDefinition(definition) }, definition);
}

export function updateContentType(project: CanonicalProject, id: string, input: unknown): ContentTypeMutationResult {
  ...
  return withValidatedProject(project, { ...project.contentTypes, [id]: serializeContentTypeDefinition(definition) }, definition);
}
...
export function removeContentType(project: CanonicalProject, id: string): ContentTypeMutationResult {
  ...
  if (!current) return { ok: false, error: { code: 'NOT_FOUND', message: `Content type ${id} was not found.` } };
  const validation = validateContentTypeDefinition(current);
  if (!validation.ok) {
    return { ok: false, error: { code: 'INVALID_DEFINITION', message: 'Existing content type definition is invalid.', issues: validation.issues } };
  }
  const recordInUse = Object.values(project.records).some((record) => record.contentTypeId === id || record.contentType === id);
  if (recordInUse) {
    return { ok: false, error: { code: 'CONTENT_TYPE_IN_USE', message: `Content type ${id} has records and cannot be deleted without an explicit migration.` } };
  }
  ...
  if (taxonomyInUse) {
    return { ok: false, error: { code: 'CONTENT_TYPE_IN_USE', message: `Content type ${id} is assigned to a taxonomy and cannot be deleted until that association is removed.` } };
  }

  ...
  group.fields.forEach((field, index) => {
    ...
    if (field.type === 'core/relation') {
      ...
      if (!relationIds.has(relationId)) {
        issues.push({
          code: 'INVALID_CONFIG',
          path: `fields.${index}.config.relationId`,
          message: `Referenced Relation ${relationId || '(empty)'} does not exist.`,
        });
      }
    }
    if (field.type === 'core/taxonomy') {
      ...
      if (!taxonomyIds.has(taxonomyId)) {
        issues.push({
          code: 'INVALID_CONFIG',
          path: `fields.${index}.config.taxonomyId`,
          message: `Referenced Taxonomy ${taxonomyId || '(empty)'} does not exist.`,
        });
      }
    }
  });

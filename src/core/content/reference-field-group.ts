...
import { createContentFieldTypeRegistry, isMf043ReferenceField } from './reference-field-types';
import { listTaxonomyDefinitions } from './taxonomy';

function contextualReferenceIssues(project: CanonicalProject, group: FieldGroupDefinition): FieldGroupValidationIssue[] {
  ...
  group.fields.forEach((field, index) => {
    ...
    if (field.type === 'core/relation') {
      ...
      if (!relationIds.has(relationId)) issues.push({ code: 'INVALID_CONFIG', path: `fields.${index}.config.relationId`, message: `Referenced Relation ${relationId || '(empty)'} does not exist.` });
    }
    if (field.type === 'core/taxonomy') {
      const taxonomyId = typeof field.config.taxonomyId === 'string' ? field.config.taxonomyId : '';
      if (!taxonomyIds.has(taxonomyId)) issues.push({ code: 'INVALID_CONFIG', path: `fields.${index}.config.taxonomyId`, message: `Referenced Taxonomy ${taxonomyId || '(empty)'} does not exist.` });
    }
  });
  return issues;
}

export function validateReferenceFieldGroupDefinition(project: CanonicalProject, input: unknown, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): FieldGroupValidationResult {
...
export function listReferenceFieldGroupDefinitions(project: CanonicalProject, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): FieldGroupDefinition[] {
  return listAdvancedFieldGroupDefinitions(project, registry).filter((group) => validateReferenceFieldGroupDefinition(project, group, registry).ok);
}
...
export function createReferenceFieldGroup(project: CanonicalProject, input: unknown, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): FieldGroupMutationResult {
...
export function updateReferenceFieldGroup(project: CanonicalProject, id: string, input: unknown, registry: FieldTypeRegistry = createContentFieldTypeRegistry()): FieldGroupMutationResult {
  const validation = validateReferenceFieldGroupDefinition(project, input, registry);
  if (!validation.ok) return invalidMutation(validation);

...
export function validateRelationDefinition(input: unknown): RelationValidationResult {
  ...
  if (input.version !== RELATION_DEFINITION_VERSION) issues.push({ code: 'INVALID_VERSION', path: 'version', message: `Relation version must be ${RELATION_DEFINITION_VERSION}.` });
  if (!RELATION_ID_PATTERN.test(id) || id.length > 64) issues.push({ code: 'INVALID_ID', path: 'id', message: 'Relation id must be kebab-case, begin with a letter and be at most 64 characters.' });
  if (!label || label.length > MAX_LABEL_LENGTH) issues.push({ code: 'INVALID_LABEL', path: 'label', message: `Relation label is required and must be at most ${MAX_LABEL_LENGTH} characters.` });
  if (description.length > MAX_DESCRIPTION_LENGTH) issues.push({ code: 'INVALID_DESCRIPTION', path: 'description', message: `Relation description must be at most ${MAX_DESCRIPTION_LENGTH} characters.` });
  if (!sourceContentTypeId) issues.push({ code: 'INVALID_SOURCE_CONTENT_TYPE', path: 'sourceContentTypeId', message: 'Source Content Type id is required.' });
  if (!targetContentTypeId) issues.push({ code: 'INVALID_TARGET_CONTENT_TYPE', path: 'targetContentTypeId', message: 'Target Content Type id is required.' });
  if (!sourceCardinality) issues.push({ code: 'INVALID_SOURCE_CARDINALITY', path: 'sourceCardinality', message: 'Source cardinality must be one or many.' });
  if (!targetCardinality) issues.push({ code: 'INVALID_TARGET_CARDINALITY', path: 'targetCardinality', message: 'Target cardinality must be one or many.' });
  if (typeof input.bidirectional !== 'boolean') issues.push({ code: 'INVALID_BIDIRECTIONAL', path: 'bidirectional', message: 'Bidirectional must be boolean.' });
...
function withRelations(project: CanonicalProject, relations: CanonicalProject['relations'], value: RelationDefinition): RelationMutationResult {
...
function validateEndpoints(project: CanonicalProject, definition: RelationDefinition): RelationMutationError | null {
  if (!contentTypeExists(project, definition.sourceContentTypeId)) return { code: 'UNKNOWN_SOURCE_CONTENT_TYPE', message: `Source Content Type ${definition.sourceContentTypeId} does not exist.` };
  if (!contentTypeExists(project, definition.targetContentTypeId)) return { code: 'UNKNOWN_TARGET_CONTENT_TYPE', message: `Target Content Type ${definition.targetContentTypeId} does not exist.` };
  return null;
}
...
export function createRelation(project: CanonicalProject, input: unknown): RelationMutationResult {
  ...
  if (definition.id in project.relations) return { ok: false, error: { code: 'DUPLICATE_ID', message: `Relation ${definition.id} already exists.` } };
...
export function updateRelation(project: CanonicalProject, id: string, input: unknown): RelationMutationResult {
  if (!(id in project.relations)) return { ok: false, error: { code: 'NOT_FOUND', message: `Relation ${id} was not found.` } };
  const validation = validateRelationDefinition(input);
  if (!validation.ok) return { ok: false, error: { code: 'INVALID_DEFINITION', message: validation.issues.map((issue) => issue.message).join(' '), issues: validation.issues } };
  const definition = validation.value;
  if (definition.id !== id) return { ok: false, error: { code: 'ID_MISMATCH', message: 'Relation id is immutable after creation.' } };
  const endpointError = validateEndpoints(project, definition);
  if (endpointError) return { ok: false, error: endpointError };
...
export function removeRelation(project: CanonicalProject, id: string): RelationMutationResult {
  ...
  if (!validation.ok) return { ok: false, error: { code: 'INVALID_DEFINITION', message: 'Existing relation definition is invalid.', issues: validation.issues } };
  ...
  if (referencedByField) return { ok: false, error: { code: 'RELATION_IN_USE', message: `Relation ${id} is referenced by a Field Group and cannot be deleted.` } };

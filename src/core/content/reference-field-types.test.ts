...
describe('MF-043 reference field types', () => {
  ...
  it('validates relation endpoint, cardinality and referenced record Content Type', () => {
    ...
    expect(validateReferenceFieldContext(project, 'product', field, ['brand-nike', 'brand-nike'])).toContain(
      'Relation product-brand allows only one referenced record from this side.',
    );

...
function validateMigration(migration: FieldTypeMigrationHook): void {
  if (
    ...
    throw new FieldTypeRegistryError(
      'INVALID_DEFINITION',
      `Field type migration ${migration.fromVersion}→${migration.toVersion} must advance exactly one version.`,
    );
  }
}
...
function validateDefinition(definition: FieldTypeDefinition): void {
  if (!FIELD_TYPE_PATTERN.test(definition.type)) {
    throw new FieldTypeRegistryError(
      'INVALID_DEFINITION',
      `Field type ${definition.type} must be namespaced as namespace/name.`,
    );
  }
  ...
  if (!definition.metadata.label.trim() || !definition.metadata.description.trim() || !definition.metadata.icon.trim()) {
    throw new FieldTypeRegistryError(
      'INVALID_DEFINITION',
      'Field type metadata requires label, description and icon.',
    );
  }
  ...
  if (!isJsonObject(definition.configSchema) || !isJsonObject(definition.defaultConfig)) {
    throw new FieldTypeRegistryError(
      'INVALID_DEFINITION',
      'Field type configSchema and defaultConfig must be portable JSON objects.',
    );
  }
  for (const feature of FIELD_TYPE_FEATURES) {
    if (!definition.features[feature]) {
      throw new FieldTypeRegistryError(
        'INVALID_DEFINITION',
        `Field type ${definition.type} is missing feature status for ${feature}.`,
      );
    }
  }
  ...
  if (!defaultConfigValidation.valid) {
    throw new FieldTypeRegistryError(
      'INVALID_DEFINITION',
      `Default config for ${definition.type}@${definition.version} is invalid.`,
    );
  }
  ...
  if (!isJsonValue(defaultValue)) {
    throw new FieldTypeRegistryError(
      'INVALID_DEFINITION',
      `Default value for ${definition.type}@${definition.version} is not portable JSON.`,
    );
  }
  const defaultValueValidation = definition.validateValue(
  ...
  if (!defaultValueValidation.valid) {
    throw new FieldTypeRegistryError(
      'INVALID_DEFINITION',
      `Default value for ${definition.type}@${definition.version} fails its validator.`,
    );
  }
}
...
export class FieldTypeRegistry {
  ...
  register(definition: FieldTypeDefinition): void {
    ...
    if (this.definitions.has(key)) {
      throw new ConflictError(`Field type ${key} is already registered.`);
    }
  ...
  validateConfig(type: string, config: unknown, version?: number): FieldTypeValidationResult {
    ...
    return definition.validateConfig(portableConfig);
  }
  ...
  validateValue(
    ...
    const portableValue = assertPortableValue(value);
    return definition.validateValue(portableValue, portableConfig);
  }
  ...
  createDefaultValue(type: string, config: unknown = {}, version?: number): JsonValue {
    ...
    if (!validation.valid) {
      throw new FieldTypeRegistryError(
        'INVALID_CONFIG',
        `Config for ${definition.type}@${definition.version} is invalid: ${validation.issues
          .map((issue) => `${issue.path}: ${issue.message}`)
          .join('; ')}`,
      );
    }
    const value = definition.createDefaultValue(structuredClone(portableConfig));
    return assertPortableValue(value);
  }
  ...
  migrateConfig(type: string, config: unknown, fromVersion: number, targetVersion?: number): JsonObject {
    ...
    if (fromVersion > resolvedTargetVersion || fromVersion < 1) {
      throw new FieldTypeRegistryError(
        'MIGRATION_PATH_MISSING',
        `Cannot migrate ${type} config from version ${fromVersion} to ${resolvedTargetVersion}.`,
      );
    }
    ...
    while (currentVersion < resolvedTargetVersion) {
      ...
      if (!hook) {
        throw new FieldTypeRegistryError(
          'MIGRATION_PATH_MISSING',
          `Missing field type migration ${type} ${currentVersion}→${currentVersion + 1}.`,
        );
      }
      ...
      if (!isJsonObject(migrated)) {
        throw new FieldTypeRegistryError(
          'MIGRATION_OUTPUT_INVALID',
          `Migration ${type} ${hook.fromVersion}→${hook.toVersion} returned non-portable config.`,
        );
      }
    ...
    if (!validation.valid) {
      throw new FieldTypeRegistryError(
        'MIGRATION_OUTPUT_INVALID',
        `Migrated config for ${type}@${resolvedTargetVersion} is invalid.`,
      );
    }

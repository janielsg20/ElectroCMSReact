...
import { createBuiltinFieldTypeDefinitions, createDefaultFieldTypeRegistry } from './builtin-field-types';
import {
  FIELD_TYPE_FEATURES,
...
function allSupportedFeatures(): Record<FieldTypeFeature, FieldTypeFeatureStatus> {
  return Object.fromEntries(FIELD_TYPE_FEATURES.map((feature) => [feature, 'supported'] as const)) as Record<FieldTypeFeature, FieldTypeFeatureStatus>;
}
...
function ratingDefinition(version = 1): FieldTypeDefinition {
  return {
    type: 'plugin/rating',
    version,
    metadata: { label: 'Rating', category: 'number', icon: 'star', description: 'External plugin rating field.', keywords: ['score'] },
    ...
    validateValue(value, config) {
      ...
      if (typeof value !== 'number') return invalidFieldTypeValue('INVALID_RATING', '$', 'Rating must be numeric or null.');
    ...
    migrations: version === 2 ? [{ fromVersion: 1, toVersion: 2, migrate(config) { return { ...config, max: typeof config.max === 'number' ? config.max * 2 : 10 }; } }] : [],
  };
}
...
describe('FieldTypeRegistry', () => {
  ...
  it('validates type-specific config and values without React or project state', () => {
    ...
    const choiceConfig = { options: [{ label: 'Draft', value: 'draft' }, { label: 'Published', value: 'published' }] };
  ...
  it('rejects malformed definitions and duplicate type versions', () => {
    ...
    expect(() => registry.register({ ...ratingDefinition(), type: 'rating' })).toThrow(FieldTypeRegistryError);
  ...
  it('rejects invalid default definitions at registration time', () => {
    const registry = new FieldTypeRegistry();
    expect(() => registry.register({ ...ratingDefinition(), defaultConfig: { max: 0 } })).toThrowError(expect.objectContaining({ registryCode: 'INVALID_DEFINITION' }));
  });
});
